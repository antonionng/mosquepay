#!/usr/bin/env node
// Seed Gift Aid demo data for a tenant mosque so the treasurer can play with
// service close + Gift Aid claim packs end-to-end.
//
// What it creates (against MOSQUE_SLUG, default: mosquepay-test-9999):
//
//   * Fills in the mosque's Gift Aid + Gift Aid pack config (reference, email).
...
//   MOSQUE_SLUG=other-mosque node scripts/seed-gift-aid-demo.mjs --reset
...
const MOSQUE_SLUG = process.env.MOSQUE_SLUG ?? "mosquepay-test-9999";
const RESET = process.argv.includes("--reset");
const DEMO_REF_PREFIX = "LP-DEMO";
const DEMO_EMAIL_PREFIX = "demo+";
const DEMO_EMAIL_DOMAIN = "mosquepay-test.test";
...
const MOSQUE_CONFIG_PATCH = {
  gift_aid_default_mode: "both",
  hmrc_charity_reference: "GA-12345-LP",
  gift_aid_pack_name: "MosquePay Test Mosque Gift Aid pack",
 ...
 */
function buildPaperScanPdf({ donor, address, dateSigned, signatureName, charityRef, mosqueName }) {
  const lines = [
    ...
    "",
    "This file was generated by the MosquePay demo seeder to represent",
...
function renderDigitalDeclarationHtml(ctx) {
  ...
  return [
    ...
    `<div class=\"meta\">${safe(ctx.mosqueName)}${
      ctx.mosqueNumber ? ` No. ${safe(ctx.mosqueNumber)}` : ""
    }${
    ...
    `</div>`,
    `<footer>This document was generated by MosquePay as a tamper-evident record of an electronic Gift Aid declaration. The SHA-256 of these bytes is recorded against the declaration row in the MosquePay database; re-hashing this file should produce the same digest. To verify: <code>shasum -a 256 thisfile.html</code>.</footer>`,
    "</body></html>",
  ].join("\n");
}
...
function buildEvidencePath({ mosqueId, declarationId, sha256, extension }) {
  const ext = extension.replace(/^\.+/, "").toLowerCase().replace(/[^a-z0-9]/g, "") || "bin";
  return `${mosqueId}/${declarationId}/${declarationId}__${sha256.slice(0, 8)}.${ext}`;
}

async function uploadEvidenceFile({ mosqueId, declarationId, bytes, mime, extension }) {
  const sha = sha256Hex(bytes);
  const path = buildEvidencePath({ mosqueId, declarationId, sha256: sha, extension });
...
async function resetDemoData(mosqueId) {
...
async function detectExistingDemo(mosqueId) {
...
async function main() {
  log(`Resolving mosque "${MOSQUE_SLUG}"...`);
  const { data: mosque, error: mosqueErr } = await supabase
    .from("mosques")
    .select("id, name, mosque_number")
    .eq("slug", MOSQUE_SLUG)
    .maybeSingle();
  if (mosqueErr) throw mosqueErr;
  if (!mosque) throw new Error(`Mosque ${MOSQUE_SLUG} not found.`);
  log(`  Mosque: ${mosque.name} (${mosque.id})`);

  if (await detectExistingDemo(mosque.id)) {
    if (!RESET) {
      console.error(
        "Demo data already exists for this mosque. Re-run with --reset to wipe and reseed."
      );
      process.exit(2);
    }
    await resetDemoData(mosque.id);
  }

  log("Updating mosque Gift Aid + Gift Aid pack config...");
  const { error: mosqueUpdErr } = await supabase
    .from("mosques")
    .update({ ...MOSQUE_CONFIG_PATCH, updated_at: new Date().toISOString() })
    .eq("id", mosque.id);
  if (mosqueUpdErr) throw mosqueUpdErr;
  ...
  const { data: insertedMembers, error: memErr } = await supabase
    .from("members")
    .upsert(memberRows, { onConflict: "mosque_id,email" })
    .select("id, email");
  ...
  for (let i = 0; i < DECLARATIONS.length; i++) {
    ...
    if (d.source === "paper") {
      const pdf = buildPaperScanPdf({
        ...
        charityRef: MOSQUE_CONFIG_PATCH.hmrc_charity_reference,
      ...
      const html = renderDigitalDeclarationHtml({
        ...
        charityReference: MOSQUE_CONFIG_PATCH.hmrc_charity_reference,
    ...
    const { error: declErr } = await supabase
      ...
      .insert({
        ...
        evidence_uploaded_by_email: isDigital ? member.email : "treasurer+demo@mosquepay-test.test",
    ...
    const eventsToInsert = [
      {
        ...
        actor_email: isDigital ? member.email : "treasurer+demo@mosquepay-test.test",
        after_state: {
    ...
    if (d.revokedAt) {
      eventsToInsert.push({
        ...
        event_type: "revoked",
        actor_kind: "admin",
        actor_email: "treasurer+demo@mosquepay-test.test",
  ...
  for (let mi = 0; mi < sortedClosed.length; mi++) {
    ...
    for (let i = 0; i < donations.length; i++) {
      ...
      const { data: payRow, error: payErr } = await supabase
        ...
        .insert({
          ...
          currency: "GBP",
          charity_name: "Mosque Charity Appeal",
          status: "succeeded",
          mooov_payment_id: mooovId,
          payment_method: paymentMethod,
          recorded_by_email: "treasurer+demo@mosquepay-test.test",
    ...
    const { data: batch, error: batchErr } = await supabase
      ...
      .insert({
        ...
        created_by_email: "treasurer+demo@mosquepay-test.test",
    ...
    await supabase
      ...
      .update({
        ...
        service_closed_by_email: "treasurer+demo@mosquepay-test.test",
