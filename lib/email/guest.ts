...
type GuestWelcomeArgs = {
  ...
  /** Optional mosque id so the email_log row carries tenant context. */
...
export async function sendGuestWelcomeEmail(args: GuestWelcomeArgs) {
  ...
  if (args.inviterName) {
    paragraphs.push(
      `${args.inviterName} has invited you to attend ${args.eventTitle} at ${args.mosqueName}, and your place is now confirmed.`,
    );
  } else {
    paragraphs.push(
      `Your place at ${args.eventTitle} (${args.mosqueName}) is now confirmed.`,
    );
  }
  ...
  const html = renderSimpleMessageEmail({
    ...
    paragraphs,
    note: "This invitation is private; please do not share it publicly. If you can no longer attend, please let the mosque know as soon as possible.",
  });
...
export async function sendGuestInviteEmail(args: GuestInviteArgs) {
  ...
  if (args.inviterName) {
    paragraphs.push(
      `${args.inviterName} would like to invite you to ${args.eventTitle} at ${args.mosqueName}.`,
    );
  } else {
    paragraphs.push(
      `${args.mosqueName} would like to invite you to ${args.eventTitle}.`,
    );
  }
  ...
  paragraphs.push(
    "Please do not share this link publicly; it is unique to you. If you can no longer attend, simply ignore this email and the mosque will know.",
  );

  const html = renderSimpleMessageEmail({
    eyebrow: "You're invited",
    title: `${args.eventTitle}`,
    preview: `${args.mosqueName} invites you to ${args.eventTitle}.`,
    greeting: `Dear ${args.toName},`,
    paragraphs,
  ...
  const result = await sendWithLog({
    ...
    html,
    text: `${paragraphs.join("\n\n")}\n`,
    metadata: { mosque_name: args.mosqueName, event_title: args.eventTitle },
  });
...
export async function sendGuestSelfRegisterEmail(args: GuestSelfRegisterArgs) {
  ...
  const paragraphs: string[] = [
    `Thank you for adding yourself to the ${args.mosqueName} newcomer directory.`,
  ];
  ...
  const html = renderSimpleMessageEmail({
    eyebrow: "You're in",
    title: `Welcome to ${args.mosqueName}`,
    preview: `Your newcomer profile at ${args.mosqueName} is ready.`,
    greeting: `Dear ${args.toName},`,
    paragraphs,
    note: "This link is unique to you and never expires unless the mosque revokes it. Bookmark it for next time.",
  });
  ...
  const result = await sendWithLog({
    ...
    dedupeKey: null,
    subject: `Welcome to ${args.mosqueName}`,
    html,
    text: `${paragraphs.join("\n\n")}\n`,
    metadata: { mosque_name: args.mosqueName },
  });
...
export async function sendGuestPostEventEmail(args: GuestPostEventArgs) {
  ...
  paragraphs.push(
    `If you would like to know about future ${args.mosqueName} events, simply reply and we will be in touch.`,
  );
  if (args.newcomerPortalUrl) {
    paragraphs.push(
