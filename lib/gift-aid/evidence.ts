 ...
 * `<lodge_id>/<declaration_id>/...` so two lodges cannot collide and the
...
export function buildEvidencePath(opts: {
  ...
  return `${opts.lodgeId}/${opts.declarationId}/${opts.declarationId}__${short}.${ext}`;
}
...
export type DigitalSignatureContext = {
  ...
  /** Charity reference, e.g. "GA-12345" or the relief chest's id. */
...
export async function persistDigitalEvidence(
  ctx: DigitalSignatureContext & { lodgeId: string },
): Promise<{
  path: string;
  bucket: string;
  sha256: string;
