 ...
 * `<mosque_id>/<declaration_id>/...` so two mosques cannot collide and the
...
export function buildEvidencePath(opts: {
  ...
  return `${opts.mosqueId}/${opts.declarationId}/${opts.declarationId}__${short}.${ext}`;
}
...
export async function persistDigitalEvidence(
  ctx: DigitalSignatureContext & { mosqueId: string },
): Promise<{
  path: string;
  bucket: string;
  sha256: string;
