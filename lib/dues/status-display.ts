// lib/dues/status-display.ts
//
// Shared display copy + colours for dues_schedules.status. Used by both
// the member-portal status card (app/(member)/member/dues) and the
// admin views (admin/members/[id], admin/dues/schedules,
...
const TABLE: Record<DuesScheduleStatus, ScheduleStatusDisplay> = {
  pending: {
  ...
  completed: {
    ...
    badgeVariant: "default",
    accentClasses: "border-slate-200 bg-slate-50",
    description: "All cycles paid for the masonic year.",
    needsAttention: false,
  },
  active_stripe: {
...
export const STATUS_SORT_PRIORITY: Record<DuesScheduleStatus, number> = {
  action_required: 0,
  past_due: 1,
  paused: 2,
  pending: 3,
