// lib/dues/year-position.ts
//
// Where is a given member, on a given day, relative to their lodge's
// masonic year? The four-quadrant model that drives the dues UX:
//
...
//
// "New" = first masonic year as a member (date_of_initiation falls inside
// the current year). "Existing" = initiated in a prior year and therefore
// expected to have paid prior years' dues.
//
...
export type YearPositionInput = {
  ...
  yearEndDate: string;
  /** ISO date of the member's initiation, if known. */
  dateOfInitiation: string | null;
  /** True when a member_dues row covering the current year is paid. */
  hasPaidCurrentYear: boolean;
  /** True when a member_dues row covering the current year exists but is unpaid. */
  hasOutstandingCurrentYear: boolean;
};

export type YearPosition = {
  quadrant: DuesQuadrant;
...
export function computeYearPosition(input: YearPositionInput): YearPosition {
  ...
  // Mid-year. Branch on whether the member is new (initiated this year)
  // or existing.
  const isNewInitiate = (() => {
    if (!input.dateOfInitiation) return false;
    const initiation = parseIsoDay(input.dateOfInitiation);
    return initiation.getTime() >= yearStart.getTime();
  })();
...
/**
 * Compute the next masonic year's start/end given the current one. Used by
 * the pay-in-advance flow when the next year's row has not yet been
 * configured by the lodge admin.
 */
