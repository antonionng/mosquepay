/**
 * Shared loader for the data the public lodge website needs that isn't
 * already on the `lodges` and `lodge_sites` rows. Used by both the
 * server-rendered routes (app/[lodgeSlug]/layout.tsx, app/(public)/page.tsx)
 * and the public JSON endpoints (app/api/lodges/[slug]/site,
 * app/api/lodges/current/site) so the public surface stays consistent
 * everywhere it is consumed.
 *
 * Privacy rules baked in here:
 *   - Events use `isPubliclyVisible` (regular lodge meetings stay hidden
 ...
 *     pinned via `lodges.current_charity_campaign_id`.
 */
...
export async function loadUpcomingPublicEvents(
  opts: Pick<LoaderOpts, "lodgeId" | "lodgeSlug">
): Promise<PublicUpcomingEvent[]> {
  const raw =
    isSupabaseConfigured() && opts.lodgeId
      ? await db
...
export async function loadPublicOfficers(
  opts: Pick<LoaderOpts, "lodgeId">
): Promise<PublicOfficerSummary[]> {
  if (!isSupabaseConfigured() || !opts.lodgeId) return [];
  try {
    return await db.listPublicOfficers(opts.lodgeId);
  } catch {
    return [];
  }
}
...
export async function loadCharityCampaign(
  opts: Pick<LoaderOpts, "lodgeId" | "currentCharityCampaignId">
