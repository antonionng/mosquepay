// Shared meeting financial reconciliation. One source of truth so the
// meeting page, the reconcile panel, the printable treasurer report, and any
...
export type PaymentMethodGroup = "cash" | "lodgepay" | "other";
...
// physical cash, LodgePay (Mooov card / QR / online), and everything else
...
export type MeetingReconciliation = {
  rafflePrice: number;
...
export function reconcileMeeting(
  ...
  for (const p of payments) {
    ...
    const general = Math.max(
      0,
      net - meetingFee - dining - guestTicket - charity - raffle,
    );
    ...
    byCategory.meeting_fee += meetingFee;
