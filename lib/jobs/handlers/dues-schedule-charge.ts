...
//      payment_id (pay_dues_<schedule_id>_<NNN>) so the cycle webhook
...
//                             handleDuesSubscriptionFailure code is
...
import { runSavedDuesCharge } from "@/lib/mooov-charges";
...
export type DuesScheduleChargeResult = {
  examined: number;
  charged: number;
...
export async function runDuesScheduleCharge(
  today: string = new Date().toISOString().slice(0, 10)
): Promise<DuesScheduleChargeResult> {
  const result: DuesScheduleChargeResult = {
    examined: 0,
    charged: 0,
    requires_action: 0,
    failed: 0,
  ...
  if (!duesSubscriptionEnabled()) {
    console.log(
      "dues.schedule.charge: skipped (LODGEPAY_DUES_SUBSCRIPTION_ENABLED is not true)"
    );
    return result;
  }

  const due = await db.getDuesSchedulesDue(today, 200);
  ...
  for (const schedule of due) {
    try {
      ...
      result.failed += 1;
      console.error("dues.schedule.charge: cycle failed for schedule", {
        schedule_id: schedule.id,
        lodge_id: schedule.lodge_id,
...
async function chargeOne(
  ...
  _today: string,
  result: DuesScheduleChargeResult
): Promise<void> {
  ...
  if (!next) {
    ...
    // instalments — likely a webhook race where the parent member_dues
    // got flipped paid. Reconcile here.
    await db.updateDuesSchedule(schedule.id, schedule.lodge_id, {
      status: "completed",
      next_charge_at: null,
    });
    result.skipped += 1;
  ...
  const lodgeRow = await db.getLodgeById(schedule.lodge_id);
  const lodgeSlug = lodgeRow?.slug ?? "";

  const cycleNumber = next.sequence;
  const seq = String(cycleNumber).padStart(3, "0");
  const paymentId = `pay_dues_${schedule.id}_${seq}`;
  const idempotencyKey = `lp:dues:cycle:${schedule.id}:${seq}`;
  const amountMinor = Math.round(next.amount * 100);
  const currency = (next.currency ?? "GBP").toUpperCase();
  const description = `Lodge dues — cycle ${cycleNumber}`;
  ...
  await db.updateInstalment(next.id, schedule.lodge_id, {
    payment_reference: paymentId,
  });

  const guestDescriptor: Record<string, unknown> = {
    source: "dues_schedule_cycle",
    schedule_id: schedule.id,
    dues_id: schedule.member_dues_id,
  ...
  const initialMetadata: Record<string, unknown> = {
    source: "lodgepay_dues_schedule_cycle",
    lodge_id: schedule.lodge_id,
    lodge_slug: lodgeSlug,
    intent: "dues_subscription_cycle",
    schedule_id: schedule.id,
    dues_id: schedule.member_dues_id,
    cycle_number: cycleNumber,
  };
  ...
  // cycle are protected by the (lodge_id, idempotency_key) unique
  ...
  const { error: insertErr } = await supa
    ...
    .upsert(
      ...
      { onConflict: "lodge_id,idempotency_key" }
    );
  if (insertErr) {
    console.error("dues.schedule.charge: payment_attempts upsert failed", {
      schedule_id: schedule.id,
      payment_id: paymentId,
      code: insertErr.code,
  ...
  try {
    const charge = await runSavedDuesCharge({
      payment_id: paymentId,
      ...
      idempotency_key: idempotencyKey,
      metadata: {
        intent: "yearly_dues_monthly_cycle",
        schedule_id: schedule.id,
        dues_id: schedule.member_dues_id,
        instalment_id: next.id,
    ...
    if (charge.status === "succeeded") {
      ...
      // run projectDuesSubscriptionCycleCaptured. The cron's role here
    ...
    if (charge.status === "failed") {
      ...
      // handleDuesSubscriptionFailure; it's idempotent on
    ...
    if (err instanceof MooovApiError) {
      console.error("dues.schedule.charge: Mooov call failed", {
        schedule_id: schedule.id,
        payment_id: paymentId,
