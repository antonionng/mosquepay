/**
 * Shared loader for the data the public church website needs that isn't
 * already on the `churches` and `church_sites` rows. Used by both the
 * server-rendered routes (app/[churchSlug]/layout.tsx, app/(public)/page.tsx)
 * and the public JSON endpoints (app/api/churches/[slug]/site,
 * app/api/churches/current/site) so the public surface stays consistent
 * everywhere it is consumed.
 *
 * Privacy rules baked in here:
 *   - Events use `isPubliclyVisible` (regular church services stay hidden
 ...
 *     pinned via `churches.current_charity_campaign_id`.
 */
...
export async function loadUpcomingPublicEvents(
  opts: Pick<LoaderOpts, "churchId" | "churchSlug">
): Promise<PublicUpcomingEvent[]> {
  const raw =
    isSupabaseConfigured() && opts.churchId
      ? await db
          .getEvents(opts.churchId, { published: true, upcoming: true })
          .catch(() => [])
      : !isSupabaseConfigured()
        ? mockDb.getEvents({
            church_slug: opts.churchSlug,
            published: true,
            upcoming: true,
          })
        : [];
...
export async function loadPublicOfficers(
  opts: Pick<LoaderOpts, "churchId">
): Promise<PublicOfficerSummary[]> {
  if (!isSupabaseConfigured() || !opts.churchId) return [];
  try {
    return await db.listPublicOfficers(opts.churchId);
  } catch {
    return [];
  }
}
...
export async function loadCharityCampaign(
  opts: Pick<LoaderOpts, "churchId" | "currentCharityCampaignId">
