# LodgePay Stripe Subscription create fixture — 2026-05-28

Locked body shape LP will POST when migrating an active monthly dues
...
- One POST per `dues_schedules` row with `cycles_completed < cycles_total`
...
- After this POST succeeds Mooov fans out `subscription.activated` →
  LP flips `dues_schedules.status='active_stripe'` and stores
...
Idempotency-Key: lp:dues:migrate:{schedule_id}
...
items[0][price_data][product_data][name]=Covenant Lodge — Annual Dues (Monthly)
items[0][price_data][product_data][metadata][lp_lodge_id]={lodge_uuid}
...
metadata[lp_lodge_id]={lodge_uuid}
metadata[lp_member_id]={member_uuid}
metadata[lp_member_dues_id]={member_dues_uuid}
...
| Field | Value | Why |
| --- | --- | --- |
| `items[0][price_data]` (inline) | per-subscription Price | LP carries variable dues amounts per lodge (£80–£250 observed). No point in a Product/Price catalog on each connected account. |
| `billing_cycle_anchor` | `dues_schedules.next_charge_at` as Unix epoch | First Stripe-driven invoice fires on the same calendar day the LP cron would have fired. Zero gap, zero double-charge. |
...
| `Idempotency-Key` | `lp:dues:migrate:{schedule_id}` | Per-schedule key. Re-running the migration script for a partial batch is safe. |
...
1. `subscription.activated` (immediate, subscription lane). Carries
   ...
   `dues_schedules.status = 'active_stripe'` and stores
   `data.subscription_id` on `dues_schedules.mooov_subscription_id`.
2. At each future billing cycle anchor, **two events**:
   - `subscription.invoice_paid` (subscription lane). Carries
     ...
     advance `dues_schedules.cycles_completed`.
...
`(lodge_id, event_id)` PK). The dual-emit lands two distinct event_ids;
...
Concrete recommendation: do NOT migrate a live monthly-dues schedule
...
| --- | --- |
| `{schedule_id}` | `c0v_dues_sched_test_001` (UUID-shaped string) |
...
| `{lodge_uuid}` | `00000000-0000-0000-0000-000000000c0v` |
| `{member_uuid}` | `00000000-0000-0000-0000-0000c0vememb` |
| `{member_dues_uuid}` | `00000000-0000-0000-0000-c0vmemberdues` |
...
- Migration script: `app/api/admin/dues/migrate-to-stripe-subscriptions/route.ts`
