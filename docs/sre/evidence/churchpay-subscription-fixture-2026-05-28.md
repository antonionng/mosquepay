# ChurchPay Stripe Subscription create fixture — 2026-05-28
...
items[0][price_data][product_data][name]=St Mary's Church — Annual Giving (Monthly)
items[0][price_data][product_data][metadata][lp_church_id]={church_uuid}
...
metadata[lp_church_id]={church_uuid}
...
| Field | Value | Why |
| --- | --- | --- |
| `items[0][price_data]` (inline) | per-subscription Price | LP carries variable giving amounts per church (£80–£250 observed). No point in a Product/Price catalog on each connected account. |
...
`(church_id, event_id)` PK). The dual-emit lands two distinct event_ids;
...
| `{church_uuid}` | `00000000-0000-0000-0000-000000000c0v` |
