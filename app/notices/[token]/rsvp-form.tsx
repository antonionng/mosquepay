...
export function NoticeRsvpForm({
  ...
  enableServiceFee,
  serviceFeeAmount,
  // serviceFeeDescription is part of the prop interface but the breakdown
  ...
  const serviceFee = feeBreakdown.items.find((i) => i.key === "levy")?.amount ?? 0;
  ...
  // in the RSVP payload (even on apologies — the member may want to drop
  ...
  async function submit() {
    ...
    try {
      ...
      const res = await fetch(
        `/api/notice/access/${encodeURIComponent(token)}/rsvp`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
