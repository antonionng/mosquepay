...
export function formFromService(service: ServiceFormService): ServiceForm {
  return {
    title: service.title,
    slug: service.slug,
    description: service.description ?? "",
    event_type: service.event_type,
    event_date: dateInput(service.event_date),
    event_time: timeInput(service.event_time),
    location: service.location ?? "",
    temple_room: service.temple_room ?? "",
    dress_code: service.dress_code ?? "",
    enable_rsvp: service.enable_rsvp,
    rsvp_deadline: dateInput(service.rsvp_deadline),
    max_attendees: service.max_attendees?.toString() ?? "",
    enable_payments: service.enable_payments,
    enable_dining_rsvp: service.enable_dining_rsvp,
    dining_price: formatMoneyInput(service.dining_price),
    dining_description: service.dining_description ?? "",
    dining_waived_for_all: service.dining_waived_for_all === true,
    enable_charity_donation: service.enable_charity_donation,
    charity_name: service.charity_name ?? "",
    charity_description: service.charity_description ?? "",
    charity_suggested_amounts:
      suggestedAmountsToInput(service.charity_suggested_amounts) ||
      DEFAULT_CHARITY_SUGGESTED,
    charity_allow_custom: service.charity_allow_custom ?? true,
    enable_raffle_donation: service.enable_raffle_donation === true,
    raffle_description:
      service.raffle_description ?? DEFAULT_RAFFLE_DESCRIPTION,
    raffle_suggested_amounts:
      suggestedAmountsToInput(service.raffle_suggested_amounts) ||
      DEFAULT_RAFFLE_SUGGESTED,
    raffle_allow_custom: service.raffle_allow_custom ?? true,
    enable_raffle_wine_pledge: service.enable_raffle_wine_pledge === true,
    raffle_wine_description:
      service.raffle_wine_description ?? DEFAULT_WINE_DESCRIPTION,
    enable_service_fee: service.enable_service_fee,
    service_fee_amount: formatMoneyInput(service.service_fee_amount),
    service_fee_description: service.service_fee_description ?? "",
    enable_guest_tickets: service.enable_guest_tickets,
    guest_ticket_price: formatMoneyInput(service.guest_ticket_price),
    guest_ticket_description: service.guest_ticket_description ?? "",
    published: service.published,
    feature_on_website: service.feature_on_website === true,
  };
}
...
function describeFeeOrigin(
  ...
  if (hasCustom) {
    return {
      resolved,
      hint: `Custom for this service: ${formatGbp(Number(trimmed))}`,
      tone: "muted",
    };
  }
        ...
        <form onSubmit={onSubmit} className="flex-1 space-y-6 overflow-y-auto p-6">
          ...
          <section className={cn("space-y-4", step !== "basics" && "hidden")}>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="space-y-2 sm:col-span-2">
                <label className="text-sm font-medium text-dash-text" htmlFor="service-title">
                  Title
                </label>
                <Input
                  id="service-title"
              ...
              <div className="space-y-2">
                <label className="text-sm font-medium text-dash-text" htmlFor="service-slug">
                  URL slug
                </label>
                <Input
                  id="service-slug"
              ...
              <div className="space-y-2">
                <label className="text-sm font-medium text-dash-text" htmlFor="service-type">
                  Type
                </label>
                <select
                  id="service-type"
              ...
              <div className="space-y-2">
                <label className="text-sm font-medium text-dash-text" htmlFor="service-date">
                  Service date
                </label>
                <Input
                  id="service-date"
              ...
              <div className="space-y-2">
                <label className="text-sm font-medium text-dash-text" htmlFor="service-time">
                  Start time
                </label>
                <Input
                  id="service-time"
              ...
              <div className="space-y-2">
                <label className="text-sm font-medium text-dash-text" htmlFor="service-location">
                  Location
                </label>
                <Input
                  id="service-location"
                  placeholder="Mark members' Hall"
                  value={form.location}
                  onChange={(event) => updateForm({ location: event.target.value })}
                />
              </div>
              <div className="space-y-2">
                <label className="text-sm font-medium text-dash-text" htmlFor="service-temple">
                  Temple room
                </label>
                <Input
                  id="service-temple"
              ...
              <div className="space-y-2 sm:col-span-2">
                <label className="text-sm font-medium text-dash-text" htmlFor="service-dress">
                  Dress code
                </label>
                <Input
                  id="service-dress"
              ...
              <div className="space-y-2 sm:col-span-2">
                <label className="text-sm font-medium text-dash-text" htmlFor="service-description">
                  Description
                </label>
                <Textarea
                  id="service-description"
                  rows={4}
                  placeholder="Optional summary printed on notice and shown on the public service page."
              ...
              {form.enable_dining_rsvp && (
                <div className="space-y-2 pl-6">
                  ...
                  <Input
                    ...
                    placeholder="e.g. Three-course special_service dinner at 7.30 pm"
                  ...
                  />
                  <p className="text-xs text-dash-muted">
                    Shown to members in the notice and on the RSVP form.
          ...
          >
            <div className="flex flex-wrap items-start justify-between gap-2">
              ...
              <Link
                href="/admin/treasurer?tab=service-dining"
                target="_blank"
            ...
            <FeeRow
              title="Member service levy"
              description="Charged to every member who attends the ceremony."
              enabled={form.enable_service_fee}
              onToggle={(value) => updateForm({ enable_service_fee: value })}
              amountId="service-fee-amount"
              amount={form.service_fee_amount}
              onAmount={(value) => updateForm({ service_fee_amount: value })}
              originHint={levyOrigin.hint}
              originTone={levyOrigin.tone}
              descriptionId="service-fee-description"
              descriptionLabel="Description (optional)"
              descriptionPlaceholder="e.g. Per-service subscription"
              descriptionValue={form.service_fee_description}
              onDescription={(value) =>
                updateForm({ service_fee_description: value })
              }
            />
            ...
            <fieldset className="space-y-3 rounded-lg border border-dash-border bg-dash-surface p-4">
              <label className="flex items-center gap-2 text-sm font-medium text-dash-text">
                ...
                />
                Collect a charity donation at this service
              </label>
            ...
            <fieldset className="space-y-3 rounded-lg border border-dash-border bg-dash-surface p-4">
              <label className="flex items-center gap-2 text-sm font-medium text-dash-text">
                ...
                />
                Sell strips of raffle tickets at this service
              </label>
              <p className="pl-6 text-xs text-dash-muted">
                Members pre-pay for a strip (or strips) of raffle tickets on
                the notice. The strips are drawn at the evening raffle on
                ...
                <div className="grid gap-4 pl-6 sm:grid-cols-2">
                  ...
                  <div className="space-y-2 sm:col-span-2">
                    ...
                    <p className="text-xs text-dash-muted">
                      ...
                      to buy a strip (or multiple strips) on the notice.
                    </p>
                  </div>
                  <label className="flex items-center gap-2 text-sm text-dash-text sm:col-span-2">
                    <input
...
function FeeSummary({
  ...
  const levyOn = form.enable_service_fee;
