...
import { isConfirmedRsvp } from "@/lib/services/attendance";
import {
  reconcileService,
  METHOD_GROUP_LABEL,
  CATEGORY_LABEL,
...
export default async function ServiceReportPage({
  ...
  const currency = "GBP";
  const recon = reconcileService(payments, { rafflePrice });
  ...
  // When a service's takings all come through the take-payment terminal there
  ...
  return (
    <div className="mx-auto max-w-[820px] px-4 py-6 text-slate-900 print:px-0 print:py-0">
      ...
      <div className="mb-5 flex items-center justify-between print:hidden">
        <p className="text-sm text-slate-500">Service treasurer&rsquo;s report</p>
        <ReportActions backHref={`/admin/services/${id}`} />
      </div>
      ...
      <div className="report-sheet rounded-xl border border-slate-200 bg-white p-8 shadow-sm print:rounded-none print:border-0 print:p-0 print:shadow-none">
            ...
            <div className="text-right text-xs text-slate-500">
              ...
              {closedAt ? (
                <p className="mt-1 font-medium text-emerald-700">
                  Service closed {longDate(closedAt)}
                </p>
              ) : (
        ...
        <Section title="Cash reconciliation (count the tin)">
          ...
          {cashCounted == null ? (
            <p className="mt-2 text-xs text-slate-500">
              Enter the counted cash on the service page to record a variance
              here.
            </p>
          ) : null}
        </Section>
        ...
        <Section title="Charity & Gift Aid">
          ...
          {collection?.gift_aid_claim_batch_id ? (
            <p className="mt-2 text-xs text-slate-500">
              ...
              {collection.gift_aid_pack_delivered_at
                ? ` · delivered to Gift Aid pack ${longDate(collection.gift_aid_pack_delivered_at)}`
                : " · not yet delivered to Gift Aid pack"}
              .
            </p>
          ) : null}
        </Section>
            ...
            <table className="w-full text-xs">
              <thead>
                <tr className="border-b border-slate-300 text-left uppercase tracking-wide text-slate-500">
                  ...
                  <th className="py-1.5">Method</th>
                  <th className="py-1.5 text-right">Dining</th>
                  <th className="py-1.5 text-right">Guest tickets</th>
              ...
              <tbody>
                {recon.payers.map((p) => (
                  <tr key={p.id} className="border-b border-slate-100">
                    ...
                    <td className="py-1.5 text-right tabular-nums">
                      {p.dining > 0 ? money(p.dining, currency) : "—"}
                    </td>
                    <td className="py-1.5 text-right tabular-nums">
                      {p.guest_ticket > 0 ? money(p.guest_ticket, currency) : "—"}
                    </td>
                    <td className="py-1.5 text-right tabular-nums">
                      {p.raffleStrips > 0
        ...
        {collection?.notes || (closedAt && "service_close_notes" in event) ? (
          <Section title="Notes">
            <p className="whitespace-pre-wrap text-sm text-slate-700">
              {collection?.notes ??
                (event as { service_close_notes?: string | null })
                  .service_close_notes ??
                ""}
            </p>
          </Section>
