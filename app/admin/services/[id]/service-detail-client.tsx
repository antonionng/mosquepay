...
type ServiceEvent = ServiceFormService;
 ...
 * a member who tapped through to checkout but never finished paying.
...
type NoticeSummary = {
  ...
  newcomer_contacts: Array<{ name: string; email?: string | null; phone?: string | null }>;
 ...
 */
export type ServiceVisibility = "draft" | "members_only" | "public";
...
export function ServiceDetailClient({
  ...
  /**
   * True when this service's event_type is private by default but could
   ...
   * the service page when supabase is configured; absent for mock mode.
   */
  closeState?: {
    ...
    gift_aid_eligible_count: number;
  ...
  /** Collected payments taken on the service date but not tagged to any service. */
  ...
  const isPast = new Date(service.event_date) < new Date();
  ...
  // the member is bringing. Guests added directly against an event with
  ...
  const paymentByRsvp = useMemo(() => {
    ...
    for (const p of payments) {
      ...
      const newcomerDate = p.completed_at ?? p.id;
      if (newcomerDate > existingDate) map.set(p.rsvp_id, p);
    }
    return map;
  }, [payments]);
  ...
  async function handleServiceSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    // services-client.tsx for the same logic — keep these two in sync.
  ...
  async function deleteService() {
    const raised = finance?.service.succeededTotal ?? 0;
    const count = finance?.service.succeededCount ?? 0;
    const paymentNote =
      count > 0
        ? `\n\n${count} payment${count === 1 ? "" : "s"} (£${raised.toFixed(2)} raised) will stay in the ledger but will no longer be linked to this service.`
        : "";
    const ok =
  ...
  async function patchService(
    ...
    try {
      const res = await fetch(`/api/events/${service.id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ ...formFromService(service), ...overrides }),
      });
      if (!res.ok) {
        const data = await res.json().catch(() => ({}));
        ...
        return [
          ...
          rsvp.payment_required
            ? rsvp.payment_completed
              ...
              : "unpaid"
            : "not required",
          payment ? moneyFmt(payment.service_fee_amount) : "",
  ...
  function exportDonations() {
    // Charity + raffle ticket strips per member. Only includes RSVPs
          ...
          <div className="flex flex-col gap-6 md:flex-row md:items-start md:justify-between">
            <div className="flex items-start gap-4">
              <div className="flex h-14 w-14 shrink-0 flex-col items-center justify-center rounded-2xl bg-gradient-to-br from-blue-500/15 to-violet-500/15 text-blue-800">
                <span className="text-[10px] font-medium uppercase leading-none">
                  {new Date(service.event_date).toLocaleDateString("en-GB", {
                    month: "short",
                  })}
                </span>
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <Button asChild variant="dashboard">
                <Link href={`/admin/services/${service.id}/notice/edit`}>
                  <Send className="mr-2 h-4 w-4" />
                  Edit notice
                </Link>
              </Button>
              <Button asChild variant="dashboard">
                <Link href={`/admin/services/${service.id}/notice`}>
                  <Eye className="mr-2 h-4 w-4" />
                  Open notice
                </Link>
              </Button>
              {!closeState?.service_closed_at ? (
                <Button
                  type="button"
                  variant="dashboard"
                  disabled={deleting}
          ...
          >
            ...
            <div className="min-w-0">
              <p className="font-semibold">
                Service readiness: {readiness.label}
              </p>
                ...
                {visibility === "members_only" && (
                  <>
                    <p className="mt-2 text-xs text-blue-900">
                      This service is published for members but not on the
                      public website, so its public URL would 404 for
                      newcomers. Use the admin preview to QA the page, and
                      share it with members through the notice.
                    </p>
                    {visibilityReason && (
                ...
                {visibility === "draft" && (
                  <>
                    <p className="mt-2 text-xs text-amber-900">
                      Draft &mdash; this service hasn&apos;t been published
        ...
        <div className="space-y-6 lg:col-span-2">
          <Card variant="panel" className="overflow-hidden p-0">
            <div className="dash-panel-header rounded-none border-dash-border bg-dash-surface-subtle">
              <div>
                <h2 className="dash-panel-header-title">Service details</h2>
              ...
              <dl className="grid gap-4 sm:grid-cols-2">
                ...
                <div className="sm:col-span-2">
                  ...
                  <dd className="mt-1 text-sm text-dash-text">
                    ...
                    {service.temple_room ? ` · ${service.temple_room}` : ""}
                  </dd>
                </div>
                <div>
                  <dt className="text-xs font-medium uppercase tracking-wide text-dash-muted">
                ...
                <div>
                  ...
                  <dd className="mt-1 text-sm text-dash-text">
                    {service.rsvp_deadline
                      ? formatDate(service.rsvp_deadline)
                      : "No deadline"}
                  </dd>
                </div>
              </dl>
          ...
          <Card variant="panel" className="overflow-hidden p-0">
            ...
            <CardContent className="space-y-4 border-t border-dash-border bg-dash-surface p-6">
              ...
              {service.enable_raffle_wine_pledge && winePledgers.length > 0 && (
        ...
        <div className="space-y-4 sm:space-y-6">
          {finance ? (
            <MoneyRaisedCard
              eventId={service.id}
              finance={finance}
              serviceHasFees={Boolean(
                service.enable_service_fee ||
                  service.enable_dining_rsvp ||
                  service.enable_guest_tickets ||
                  service.enable_charity_donation ||
                  service.enable_raffle_donation,
              )}
              enabled={{
                serviceFee: Boolean(service.enable_service_fee),
                dining: Boolean(service.enable_dining_rsvp),
                guestTicket: Boolean(service.enable_guest_tickets),
                charity: Boolean(service.enable_charity_donation),
                raffle: Boolean(service.enable_raffle_donation),
              }}
            />
          ) : null}
          ...
          {closeState ? (
            <ServiceClosePanel
              eventId={service.id}
              isPast={isPast}
              closedAt={closeState.service_closed_at}
              closedByEmail={closeState.service_closed_by_email}
              charityAmount={closeState.charity_amount}
              charityCount={closeState.charity_count}
              giftAidEligibleAmount={closeState.gift_aid_eligible_amount}
              giftAidEligibleCount={closeState.gift_aid_eligible_count}
              ...
              reliefChestDeliveredAt={closeState.gift_aid_pack_delivered_at ?? null}
          ...
          <Card variant="panel" className="overflow-hidden p-0">
            ...
            <CardContent className="border-t border-dash-border bg-dash-surface p-5">
              <dl className="space-y-3 text-sm">
                <ConfigRow
                  label="RSVP"
                  value={service.enable_rsvp ? "Enabled" : "Off"}
                />
                <ConfigRow
                  label="Dining"
                  value={
                    service.enable_dining_rsvp
                      ? formatPrice(service.dining_price)
                      : "Off"
                  }
                  hint={service.dining_description ?? undefined}
                />
                <ConfigRow
                  label="Payments"
                  value={service.enable_payments ? "Enabled" : "Off"}
                />
                <ConfigRow
                  label="Service fee"
                  value={
                    service.enable_service_fee
                      ? formatPrice(service.service_fee_amount)
                      : "Off"
                  }
                  hint={service.service_fee_description ?? undefined}
                />
                <ConfigRow
                  label="Charity"
                  value={
                    service.enable_charity_donation
                      ? service.charity_name ?? "Enabled"
                      : "Off"
                  }
                  hint={service.charity_description ?? undefined}
                />
                <ConfigRow
                  label="Raffle ticket strips"
                  value={service.enable_raffle_donation ? "Enabled" : "Off"}
                  hint={service.raffle_description ?? undefined}
                />
                <ConfigRow
                  label="Wine pledge"
                  value={
                    service.enable_raffle_wine_pledge ? "Enabled" : "Off"
                  }
                  hint={service.raffle_wine_description ?? undefined}
                />
                <ConfigRow
                  label="Guest tickets"
                  value={
                    service.enable_guest_tickets
                      ? formatPrice(service.guest_ticket_price)
                      : "Off"
                  }
                  hint={service.guest_ticket_description ?? undefined}
                />
                <ConfigRow
                  label="Max attendees"
                  value={service.max_attendees?.toString() ?? "No limit"}
                />
              </dl>
            </CardContent>
          </Card>
          ...
          <Card variant="panel" className="overflow-hidden p-0">
            <div className="dash-panel-header rounded-none border-dash-border bg-dash-surface-subtle">
              <h3 className="dash-panel-header-title">Notice</h3>
            </div>
                ...
                <>
                  ...
                  {notice.dining_time && (
                    <div className="flex items-center justify-between text-dash-text">
                      ...
                      <span className="font-medium">{notice.dining_time}</span>
                    </div>
                  )}
                  <div className="flex items-center justify-between text-dash-text">
                    <span className="text-dash-muted">Agenda items</span>
                    <span className="font-medium">
                      {notice.agenda_items.length}
                    </span>
                  </div>
                  <div className="flex items-center justify-between text-dash-text">
                    <span className="text-dash-muted">Menu items</span>
                    <span className="font-medium">
                      {notice.menu_items.length}
                    </span>
                  </div>
                  {(notice.newcomer_contacts.length > 0 ||
                    notice.newcomer_contact_name) && (
                    <div className="flex items-center justify-between text-dash-text">
                      <span className="text-dash-muted">VO</span>
                      <span className="truncate font-medium text-right">
                        {notice.newcomer_contacts.length > 1
                          ? `${notice.newcomer_contacts.length} officers`
                          : notice.newcomer_contacts[0]?.name ??
                            notice.newcomer_contact_name}
                      </span>
                    </div>
                  )}
                  {notice.next_service_date && (
                    <div className="flex items-center justify-between text-dash-text">
                      <span className="text-dash-muted">Next service</span>
                      <span className="font-medium">
              ...
              <div className="pt-2">
                <Button asChild variant="dashboard" size="sm" className="w-full">
                  <Link href={`/admin/services/${service.id}/notice/edit`}>
                    ...
                    {notice ? "Edit notice" : "Draft notice"}
                  </Link>
                </Button>
              </div>
              {latestSend && (
          ...
          <Card variant="panel" className="overflow-hidden p-0">
            ...
            <CardContent className="space-y-3 border-t border-dash-border bg-dash-surface p-5 text-sm">
              ...
              {canFeatureOnWebsite && service.published && (
                <div className="rounded-lg border border-dash-border bg-dash-surface-subtle/60 p-3">
                  <label className="flex cursor-pointer items-start gap-2 text-xs text-dash-text">
                    <input
                      type="checkbox"
                      className="mt-0.5"
                      checked={service.feature_on_website}
                      onChange={(event) =>
                    ...
                    <span className="min-w-0">
                      ...
                      <span className="mt-0.5 block text-dash-muted">
                        ...
                        special_services or other services you want newcomers to
                        see.
                      </span>
                    </span>
                  </label>
...
function ReconcileCard({
  ...
  const recon = useMemo(
    () => reconcileService(payments, { rafflePrice }),
  ...
  const reportHref = `/admin/services/${eventId}/report?strip=${encodeURIComponent(
    String(rafflePrice),
        ...
        {unattributed && unattributed.count > 0 ? (
          ...
          >
            ...
            <span>
              ...
              date aren&rsquo;t tagged to a service.{" "}
...
function derivePaymentCategory(p: PaymentEntry): PaymentsTabCategory {
  const buckets: Array<[PaymentsTabCategory, number]> = [
    ...
    ["service_fee", p.service_fee_amount ?? 0],
...
/**
 * Service-scoped "Payments" tab. Mirrors the main /admin/payments section but
 * pinned to this service: a same-day reconciliation panel (bulk-associate
 * untagged takings), a revenue-by-type + method breakdown, and a per-payment
 * ledger with inline re-categorise / re-link / detach.
 */
function ServicePaymentsTab({
  eventId,
  eventTitle,
  eventDate,
  payments,
        ...
        <CardContent className="space-y-5 border-t border-dash-border bg-dash-surface p-5">
          {recon.collectedCount === 0 ? (
            <p className="rounded-md border border-dashed border-dash-border bg-dash-surface-subtle/40 px-3 py-6 text-center text-sm text-dash-text-muted">
              No payments tagged to this service yet.
              {sameDayUntagged.length === 0
...
function SameDayAssociatePanel({
  ...
  // service" — uncheck the odd one out.
  ...
  const associate = async () => {
    ...
    try {
      const res = await fetch(
        `/api/admin/services/${eventId}/associate-payments`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
        ...
        <div className="flex items-start gap-2">
          ...
          <div>
            ...
            <p className="mt-0.5 text-xs text-amber-800">
              ...
              tagged to any service. Tick the ones that belong to this evening
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <label className="block text-xs">
              ...
              <select
                ...
                className="h-9 w-full rounded-lg border border-dash-border bg-dash-surface px-2 text-sm text-dash-text outline-none focus:border-dash-ring"
              >
                <option value="">No service (detach)</option>
                {events.map((ev) => (
...
/**
 * Money raised summary on the service detail page. Surfaces:
 *
 *   - The headline total raised for THIS service (succeeded only).
 *   - Pending top-up (in-flight QR + unsettled cash) — useful in the
 *     hours after a service before settlement.
 *   - Sub-amount breakdown so the treasurer can see how the total split
 *     across service fee / dining / charity / raffle / guest tickets.
...
function MoneyRaisedCard({
  ...
  serviceHasFees: boolean;
  /**
   * Which fee buckets this service has switched on. When a bucket is
  ...
  // service doesn't have switched on) lives only in total_amount. Surface
  ...
  const uncategorised = Math.max(0, service.succeededTotal - categorisedTotal);
  const breakdownAll: Array<[string, number, boolean]> = [
    ["Service fee", service.serviceFee, enabled.serviceFee],
    ["Dining", service.dining, enabled.dining],
    ["Guest tickets", service.guestTicket, enabled.guestTicket],
    ["Charity", service.charity, enabled.charity],
    ["Raffle", service.raffle, enabled.raffle],
  ...
  // service toggles them on, even before the first payment lands.
      ...
      <CardContent className="space-y-4 border-t border-dash-border bg-dash-surface p-5">
        <div>
          ...
          <p className="mt-1 text-2xl font-semibold text-dash-text">
            {formatMoney(service.succeededTotal, currency)}
          </p>
          {service.pendingTotal > 0 ? (
            <p className="mt-0.5 text-xs text-amber-700">
              + {formatMoney(service.pendingTotal, currency)} pending (
              {service.pendingCount} unsettled)
            </p>
          ) : null}
          <p className="mt-0.5 text-xs text-dash-text-muted">
            {service.succeededCount} payment
            {service.succeededCount === 1 ? "" : "s"} settled
            {service.refunded > 0
              ? ` · ${formatMoney(service.refunded, currency)} refunded`
              : ""}
          </p>
        </div>
        ...
        {!hasServiceActivity ? (
          <p className="rounded-md border border-dashed border-dash-border bg-dash-surface-subtle/40 px-3 py-2 text-xs text-dash-text-muted">
            {serviceHasFees
              ...
              : "No payments yet. Use the buttons below to record charity, raffle, or guest-ticket money against this service."}
          </p>
        ) : null}
