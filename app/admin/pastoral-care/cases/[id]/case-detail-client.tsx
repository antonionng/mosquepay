...
export function CaseDetailClient({
  pastoralCase: initial,
  visits: initialVisits,
  member,
  auditLogs,
  relatedAlerts,
}: {
  pastoralCase: PastoralCareCase;
  visits: Visit[];
  member: Member | null;
  auditLogs: AuditLog[];
  relatedAlerts: Alert[];
  ...
  const [pastoralCase, setPastoralCareCase] = useState(initial);
  ...
  async function patch(updates: Partial<PastoralCareCase>) {
    setBusy(true);
    setFeedback(null);
    try {
      const res = await fetch(`/api/pastoral/cases/${pastoralCase.id}`, {
        method: "PATCH",
  ...
  async function logVisit(e: React.FormEvent) {
    ...
    setFeedback(null);
    try {
      const res = await fetch(`/api/pastoral/cases/${pastoralCase.id}/newcomerss`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  async function actOnAlert(id: string, status: "acknowledged" | "snoozed" | "resolved") {
    try {
      const res = await fetch(`/api/pastoral/alerts/${id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
      ...
      <div className="admin-page-head">
        <div className="min-w-0 flex-1">
          <h1 className="admin-page-title flex items-center gap-2">
            ...
            {pastoralCase.contact_name}
          </h1>
          <p className="admin-page-copy capitalize">
            {pastoralCase.case_type} · opened {formatDate(pastoralCase.opened_at)}
          </p>
        </div>
        <Button asChild variant="outline" size="sm">
          <Link href="/admin/pastoral_care" className="gap-2">
            <ArrowLeft className="h-4 w-4" /> Back to pastoral_care
          </Link>
        </Button>
      </div>
          ...
          <Card variant="panel" className="space-y-4 p-5">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <Field label="Contact name">
                <input
                  ...
                  defaultValue={pastoralCase.contact_name}
                  onBlur={(e) =>
                    e.target.value !== pastoralCase.contact_name &&
              ...
              <Field label="Status">
                <select
                  className={INPUT_CLASS}
                  value={pastoralCase.status}
                  onChange={(e) =>
                    patch({ status: e.target.value as PastoralCareCase["status"] })
                  }
                  disabled={busy}
                >
              ...
              <Field label="Severity">
                <select
                  className={INPUT_CLASS}
                  value={pastoralCase.severity}
                  onChange={(e) =>
                    patch({ severity: e.target.value as PastoralCareCase["severity"] })
                  }
                  disabled={busy}
                >
              ...
              <Field label="Case type">
                <select
                  className={INPUT_CLASS}
                  value={pastoralCase.case_type}
                  onChange={(e) =>
                    patch({ case_type: e.target.value as PastoralCareCase["case_type"] })
                  }
                  disabled={busy}
                >
              ...
              <Field label="Phone">
                <input
                  className={INPUT_CLASS}
                  defaultValue={pastoralCase.contact_phone ?? ""}
                  onBlur={(e) =>
                    e.target.value !== (pastoralCase.contact_phone ?? "") &&
              ...
              <Field label="Email">
                <input
                  type="email"
                  className={INPUT_CLASS}
                  defaultValue={pastoralCase.contact_email ?? ""}
                  onBlur={(e) =>
                    e.target.value !== (pastoralCase.contact_email ?? "") &&
              ...
              <Field label="Next action">
                <input
                  className={INPUT_CLASS}
                  defaultValue={pastoralCase.next_action ?? ""}
                  onBlur={(e) =>
                    e.target.value !== (pastoralCase.next_action ?? "") &&
              ...
              <Field label="Next action due">
                <input
                  type="date"
                  className={INPUT_CLASS}
                  defaultValue={pastoralCase.next_action_due?.slice(0, 10) ?? ""}
                  onBlur={(e) =>
                    e.target.value !== (pastoralCase.next_action_due?.slice(0, 10) ?? "") &&
            ...
            <Field label="Summary">
              <textarea
                ...
                defaultValue={pastoralCase.summary ?? ""}
                onBlur={(e) =>
                  e.target.value !== (pastoralCase.summary ?? "") &&
        ...
        <aside className="space-y-4">
          <Card variant="panel" className="space-y-3 p-5">
            ...
            <div className="flex flex-wrap gap-2">
              <Badge
                variant={STATUS_TONE[pastoralCase.status]}
                className="capitalize"
              >
                {pastoralCase.status}
              </Badge>
              <Badge
                variant={SEVERITY_TONE[pastoralCase.severity]}
                className="capitalize"
              >
                {pastoralCase.severity}
              </Badge>
            </div>
            <Button
              variant="outline"
              ...
              onClick={() =>
                patch({
                  status: pastoralCase.status === "closed" ? "open" : "closed",
                })
              }
            >
              {pastoralCase.status === "closed" ? (
                <>
