...
export function PastoralCareClient({
  cases,
  alerts,
  register,
  members,
  ...
  async function saveRegisterEdit() {
    ...
    setFeedback(null);
    try {
      const res = await fetch(`/api/pastoral/register/${editRegister.id}`, {
        method: "PATCH",
  ...
  async function regenerateAlerts() {
    ...
    setFeedback(null);
    try {
      const res = await fetch("/api/pastoral/alerts", { method: "POST" });
  ...
  async function createCase(e: React.FormEvent) {
    ...
    try {
      ...
      const res = await fetch("/api/pastoral/cases", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  async function logVisit(e: React.FormEvent) {
    ...
    setBusy(true);
    try {
      const res = await fetch(`/api/pastoral/cases/${newVisit.case_id}/newcomerss`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  async function createRegister(e: React.FormEvent) {
    ...
    setBusy(true);
    try {
      const res = await fetch("/api/pastoral/register", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  async function changeCaseStatus(id: string, status: "open" | "monitoring" | "closed") {
    setBusy(true);
    try {
      const res = await fetch(`/api/pastoral/cases/${id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ status }),
      });
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">PastoralCare</h1>
          <p className="mt-1 text-sm text-slate-500">
            PastoralCare cases, visits log, registers, and care alerts. Permissioned
            so only the pastoral_care and senior officers can see sensitive details.
          </p>
        </div>
                  ...
                  cases.map((c) => (
                    <TableRow key={c.id} className="hover:bg-slate-50">
                      <TableCell className="text-sm">
                        <Link
                          href={`/admin/pastoral_care/cases/${c.id}`}
                      ...
                      <TableCell>
                        <div className="flex justify-end gap-1">
                          <Button asChild size="sm" variant="outline">
                            <Link href={`/admin/pastoral_care/cases/${c.id}`}>
