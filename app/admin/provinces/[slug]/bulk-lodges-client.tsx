...
export function BulkLodgesClient({
  provinceId,
  provinceName,
}: {
  provinceId: string;
  ...
  function downloadTemplate() {
    ...
    a.href = url;
    a.download = `${provinceName.toLowerCase().replace(/\s+/g, "-")}-lodges-template.csv`;
    a.click();
  ...
  async function onFile(file: File | null) {
    ...
    setRows(parsed);
    setFeedback(`Loaded ${parsed.length} lodges from ${file.name}.`);
  }

  async function submit() {
    if (validRows.length === 0) {
      setFeedback("Add at least one lodge with a name.");
      return;
    }
    setBusy(true);
    setResults(null);
    try {
      const res = await fetch(`/api/provinces/${provinceId}/lodges/bulk`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ lodges: validRows }),
      });
      ...
      setFeedback(
        `Created ${data.created?.length ?? 0} lodges. ${data.errors?.length ?? 0} errors.`
      );
      router.refresh();
    } catch (error) {
  ...
  if (!open) {
    return (
      <Button variant="outline" onClick={() => setOpen(true)}>
        <Plus className="mr-2 h-4 w-4" /> Bulk add lodges
      </Button>
    );
  }
      ...
      <div className="flex items-center justify-between border-b border-slate-100 px-5 py-3">
        <div>
          <h2 className="text-base font-semibold text-slate-900">
            Bulk add lodges to {provinceName}
          </h2>
      ...
      <div className="space-y-3 p-5">
        ...
        <div className="flex justify-between">
          ...
          <Button onClick={submit} disabled={busy}>
            ...
            Create {validRows.length} lodges
          </Button>
        </div>
        ...
        {results && (
          <div className="space-y-2 rounded-lg border border-slate-200 bg-slate-50 p-3 text-xs">
            <p className="font-semibold text-slate-700">
              Created {results.created.length} lodges
            </p>
