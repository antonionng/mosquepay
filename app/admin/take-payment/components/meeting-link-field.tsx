...
// Prominent, always-visible meeting selector for the POS. The single biggest
// reconciliation problem is takings that never got tagged to a meeting, so we
// surface this above the fold (not buried in "Extras") and pre-select today's
// meeting when there's an obvious one. The duty officer confirms rather than
// has to remember.
...
export function MeetingLinkField({
  ...
  /** True when we pre-selected today's meeting (vs the operator choosing). */
  ...
  return (
    ...
    >
      <div className="mb-2 flex items-center gap-2">
        ...
        >
          {selected ? "Linked to a meeting" : "Which meeting is this for?"}
        </span>
      ...
      >
        ...
        <SelectContent>
          <SelectItem value={NO_EVENT_VALUE}>Not for a meeting</SelectItem>
          {events.map((event) => (
      ...
      >
        {selected
          ? "This payment will roll into the meeting's totals and reconciliation."
          : "Linking now saves reconciling later. Pick the meeting these takings are for, or leave as “Not for a meeting”."}
      </p>
    </div>
  );
}
