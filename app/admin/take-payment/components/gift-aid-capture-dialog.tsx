...
// On-the-day Gift Aid declaration capture for the take-payment surface.
//
// Treasurer is sitting at the festive board with a stack of paper
// declarations and a phone camera. After they log a £20 cash charity entry
// for a Brother who has no declaration on file, this dialog lets them:
//
//   1. Snap a photo of the wet-ink slip (or upload a PDF).
//   2. Confirm address and tick the "I hold the original" attestation.
//   3. Submit. The declaration is recorded against the lodge with the
//      member or guest id and the same email used on the payment, AND the
//      just-logged payment is retroactively projected into a Gift Aid
//      donation row so the per-meeting batch will pick it up.
//
// All four moving parts (file upload, declaration row, donation backfill,
// audit event) are wrapped server-side in two POSTs:
//
//   POST /api/admin/gift-aid/declarations            -> creates the row
//   POST /api/admin/take-payment/<id>/gift-aid-attach -> backfills donation
...
export function GiftAidCaptureDialog({
  open,
  onOpenChange,
  paymentId,
  payer,
  ...
  async function submit(event: React.FormEvent) {
    ...
    if (!donorName.trim() || !donorEmail.trim() || !line1.trim() || !city.trim() || !postcode.trim()) {
    ...
    try {
      ...
      // claim batch picks it up. Server enforces lodge + amount sanity.
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          <DialogTitle>Add paper Gift Aid declaration</DialogTitle>
          <DialogDescription>
            Photograph the signed slip and capture the donor address so the
            lodge can reclaim 25 percent.
          </DialogDescription>
        </DialogHeader>
        <form onSubmit={submit} className="space-y-4">
          ...
          <label className="flex items-start gap-2 text-sm text-slate-700">
            ...
            <span>
              ...
              and will file it with the lodge records.
            </span>
          </label>
