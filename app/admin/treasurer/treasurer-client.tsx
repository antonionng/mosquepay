...
function DuesPaymentMethodPanel({
  ...
  const totalTagged =
    breakdown.online_subscription +
    breakdown.bacs +
    breakdown.paid_in_full +
    breakdown.fee_waived;
  const totalRows = totalTagged + breakdown.unset;
    ...
    <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      <div className="flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-0.5 text-sm text-slate-500">
            How {activeMembers} active{" "}
            {activeMembers === 1 ? "member is" : "members are"} paying this
            year. Tag &quot;unset&quot; members from their profile to keep
            this view honest.
          </p>
        </div>
        {breakdown.bacs > 0 && breakdown.bacs_monthly_total > 0 ? (
          <div className="rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-900">
            <span className="font-semibold">
              £{breakdown.bacs_monthly_total.toFixed(2)}
            </span>{" "}
            / month expected via BACS
          </div>
        ) : null}
      </div>
      ...
      <div className="mt-4 grid grid-cols-2 gap-2 lg:grid-cols-5">
        ...
        <MethodTile
          ...
          hint={
            breakdown.bacs > 0
              ? `£${breakdown.bacs_monthly_total.toFixed(2)} / month`
        ...
        <MethodTile
          ...
          tone={breakdown.unset > 0 ? "red" : "slate"}
          hint={
            breakdown.unset > 0
...
function MethodTile({
  ...
  return (
    <div className={cn("rounded-xl border p-3", ringClass)}>
      <div className="flex items-center justify-between gap-2">
