...
export function BulkChurchesClient({
  networkId,
  networkName,
}: {
  networkId: string;
  ...
  function downloadTemplate() {
    ...
    a.href = url;
    a.download = `${networkName.toLowerCase().replace(/\s+/g, "-")}-churches-template.csv`;
    a.click();
  ...
  async function onFile(file: File | null) {
    ...
    setRows(parsed);
    setFeedback(`Loaded ${parsed.length} churches from ${file.name}.`);
  }

  async function submit() {
    if (validRows.length === 0) {
      setFeedback("Add at least one church with a name.");
      return;
    }
    setBusy(true);
    setResults(null);
    try {
      const res = await fetch(`/api/networks/${networkId}/churches/bulk`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ churches: validRows }),
      });
      ...
      setFeedback(
        `Created ${data.created?.length ?? 0} churches. ${data.errors?.length ?? 0} errors.`
      );
      router.refresh();
    } catch (error) {
  ...
  if (!open) {
    return (
      <Button variant="outline" onClick={() => setOpen(true)}>
        <Plus className="mr-2 h-4 w-4" /> Bulk add churches
      </Button>
    );
  }
      ...
      <div className="flex items-center justify-between border-b border-slate-100 px-5 py-3">
        <div>
          <h2 className="text-base font-semibold text-slate-900">
            Bulk add churches to {networkName}
          </h2>
      ...
      <div className="space-y-3 p-5">
        ...
        <div className="flex justify-between">
          ...
          <Button onClick={submit} disabled={busy}>
            ...
            Create {validRows.length} churches
          </Button>
        </div>
        ...
        {results && (
          <div className="space-y-2 rounded-lg border border-slate-200 bg-slate-50 p-3 text-xs">
            <p className="font-semibold text-slate-700">
              Created {results.created.length} churches
            </p>
