...
import { NewcomerRegisterForm } from "./register-form";

export const dynamic = "force-dynamic";

export const metadata: Metadata = {
  title: "Join the newcomer directory",
...
export default async function PublicVisitPage({
  ...
  if (isSupabaseConfigured()) {
    const row = await db.getMosqueBySlug(slug);
    if (row) {
      mosque = {
        slug: row.slug,
        name: row.name,
        logo_url: row.logo_url,
        newcomer_notice: row.newcomer_notice,
        accepts_self_registration: row.accepts_self_registration ?? false,
      };
      if (mosque.accepts_self_registration) {
    ...
    const row = mockDb.getMosqueBySlug(slug);
    if (row) {
      mosque = {
        slug: row.slug,
        name: row.name,
      ...
      <main className="min-h-screen bg-slate-50 px-4 py-12">
        <div className="mx-auto max-w-md rounded-3xl border border-slate-200 bg-white p-8 text-center shadow-sm">
          ...
          <p className="mt-3 text-sm leading-relaxed text-slate-600">
            {mosque.name} is not accepting public newcomer registrations at the
            moment. Please contact the mosque secretary if you would like to
            visit.
          </p>
        </div>
      </main>
      ...
      <article className="mx-auto max-w-3xl space-y-8">
        <header className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-10">
          ...
          <h1 className="mt-6 text-3xl font-semibold tracking-tight text-slate-950 sm:text-4xl">
            Join the {mosque.name} newcomer directory
          </h1>
          <p className="mt-4 text-base leading-relaxed text-slate-600">
            Add yourself to our directory and we will send you a private link
            you can use to book yourself into open events. Newcomer members
        ...
        <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-10">
          ...
          <p className="mt-2 text-sm text-slate-500">
            Your details are kept private and shared only with the mosque
            officers responsible for hosting.
          </p>
          <div className="mt-6">
            <NewcomerRegisterForm mosqueSlug={mosque.slug} />
          </div>
        </section>
        ...
        <p className="text-center text-xs text-slate-400">
          By submitting you agree to be added to the mosque&apos;s private
          newcomer directory. You can ask the secretary to remove your details
          at any time.
        </p>
      </article>
    </main>
