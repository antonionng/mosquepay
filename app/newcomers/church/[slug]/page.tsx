...
export default async function PublicVisitPage({
  ...
  if (isSupabaseConfigured()) {
    const row = await db.getChurchBySlug(slug);
    if (row) {
      church = {
        slug: row.slug,
        name: row.name,
      ...
      if (church.accepts_self_registration) {
    ...
    const row = mockDb.getChurchBySlug(slug);
    if (row) {
      church = {
        slug: row.slug,
        name: row.name,
      ...
      <main className="min-h-screen bg-slate-50 px-4 py-12">
        <div className="mx-auto max-w-md rounded-3xl border border-slate-200 bg-white p-8 text-center shadow-sm">
          ...
          <p className="mt-3 text-sm leading-relaxed text-slate-600">
            {church.name} is not accepting public newcomer registrations at the
            moment. Please contact the church secretary if you would like to
            visit.
          </p>
        </div>
      </main>
      ...
      <article className="mx-auto max-w-3xl space-y-8">
        <header className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-10">
          ...
          <h1 className="mt-6 text-3xl font-semibold tracking-tight text-slate-950 sm:text-4xl">
            Join the {church.name} newcomer directory
          </h1>
        ...
        <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-10">
          ...
          <p className="mt-2 text-sm text-slate-500">
            Your details are kept private and shared only with the church
            officers responsible for hosting.
          </p>
          <div className="mt-6">
            <NewcomerRegisterForm churchSlug={church.slug} />
          </div>
        </section>
        ...
        <p className="text-center text-xs text-slate-400">
          By submitting you agree to be added to the church&apos;s private
