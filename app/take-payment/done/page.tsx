...
//
// Server component: looks up the payment_attempt + mosque for friendly display.
...
async function loadSummary(paymentId: string | null) {
  ...
  try {
    ...
    const { data: attempt } = await supa
      ...
      .select("payment_id, status, amount, currency, mosque_id")
    ...
    try {
      const mosque = await db.getMosqueById(attempt.mosque_id);
            ...
            <dl className="mt-6 space-y-2 rounded-2xl bg-slate-50 px-5 py-4 text-left text-sm">
              ...
              {summary.mosqueName ? (
                <div className="flex items-center justify-between">
                  ...
                  <dd className="font-medium text-slate-800">{summary.mosqueName}</dd>
                </div>
              ) : null}
            </dl>
          ) : null}
