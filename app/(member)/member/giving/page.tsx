...
export default function MemberGivingPage() {
  const [giving, setGiving] = useState<GivingInfo | null>(null);
  ...
  async function handlePay(mode: "payment" | "subscription") {
    ...
    setError(null);
    try {
      const res = await fetch("/api/giving/pay", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          giving_id: giving.givingId,
          member_email: giving.memberEmail,
          member_name: giving.memberName,
          mode,
  ...
  // /api/giving/subscription-preview endpoint so the member sees the full
  ...
  async function openSubscriptionPreview(cadence?: Cadence) {
    ...
    setError(null);
    try {
      const res = await fetch("/api/giving/subscription-preview", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          giving_id: giving.givingId,
  ...
  const isPaid = giving?.status === "paid";
  const outstanding = giving ? giving.annualAmount - giving.paidAmount : 0;
  const advance = giving?.advance ?? null;
  const schedule = giving?.schedule ?? null;
  ...
  async function handleCancelSchedule() {
    ...
    try {
      const res = await fetch(
        `/api/giving/schedules/${schedule.id}/cancel`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
  ...
  async function handlePayInAdvance() {
    ...
    setError(null);
    try {
      const mintRes = await fetch("/api/giving/pay-in-advance", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      });
      const mintData = await mintRes.json().catch(() => ({}));
      if (!mintRes.ok || !mintData.member_giving_id) {
        throw new Error(
          mintData.error ?? "Could not start advance payment."
        );
      }
      const payRes = await fetch("/api/giving/pay", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          giving_id: mintData.member_giving_id,
          member_email: giving?.memberEmail,
          member_name: giving?.memberName,
          mode: "one_off",
        }),
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
        ...
        <>
            ...
            >
              ...
              {isPaid ? (
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-lg font-bold text-emerald-900">All giving paid</p>
                  ...
                  <p className="text-sm text-amber-700 mt-1">
                    ...
                    {giving?.dueDate && (
                      <span className="flex items-center gap-1 mt-1">
                        ...
                        Due by{" "}
                        {new Date(giving.dueDate).toLocaleDateString("en-GB", {
                          day: "numeric",
                          month: "long",
                          year: "numeric",
                        })}
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
              ...
              <div className="flex items-baseline gap-1 mb-1">
                ...
                <span className="text-3xl font-bold text-slate-900">
                  {giving?.annualAmount.toFixed(2)}
                </span>
              </div>
              <p className="text-sm text-slate-500">per year</p>

              {giving && giving.paidAmount > 0 && giving.paidAmount < giving.annualAmount && (
                <div className="mt-4">
                  <div className="flex items-center justify-between text-xs text-slate-500 mb-1.5">
                    <span>Paid: £{giving.paidAmount.toFixed(2)}</span>
                    <span>
                      {Math.round((giving.paidAmount / giving.annualAmount) * 100)}%
                    </span>
                  </div>
                  <div className="h-2 rounded-full bg-slate-100 overflow-hidden">
                    <div
                      ...
                      style={{
                        width: `${(giving.paidAmount / giving.annualAmount) * 100}%`,
                      }}
                    />
                  </div>
                </div>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-slate-600">
                          ...
                          {giving.instalmentFrequency === "quarterly"
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="font-semibold text-slate-900">
                      {advance.alreadyPaid
                        ? `${advance.nextYearLabel} giving are paid in advance`
                        : `Pay your ${advance.nextYearLabel} giving now`}
                    </p>
                    <p className="text-sm text-slate-600">
                      {advance.alreadyPaid
                        ? `Thank you — you're set for the next giving year.`
          ...
          <div className="rounded-2xl border border-slate-200 bg-white shadow-sm">
            ...
            <div className="divide-y divide-slate-100">
              {giving?.history && giving.history.length > 0 ? (
                giving.history.map((item) => (
                ...
                <div className="flex flex-col items-center justify-center py-12 text-center">
                  ...
                  <p className="text-xs text-slate-400 mt-1">
                    Past giving payments will appear here
                  </p>
                </div>
              )}
            </div>
      ...
      <ConfirmActionDialog
        ...
        confirmLabel="Cancel monthly giving"
        ...
        {isActionRequired ? (
          <div className="flex flex-col gap-3 rounded-xl border border-amber-300 bg-amber-100/50 p-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <Button
              ...
              className="shrink-0"
            >
              <a href={`/giving/schedules/${schedule.id}/resume`}>
...
// SubscriptionPreviewDialog
// ---------------------------------------------------------------------------
// Shown after the member clicks "Set Up Instalments" on /member/giving.
...
// computed by /api/giving/subscription-preview against the same code path
// as /api/giving/pay so the preview cannot disagree with the actual charge.
            ...
            <DialogTitle className="text-lg font-semibold text-dash-text">
              {plan
                ? plan.cadence === "quarterly"
                  ? "Your quarterly giving plan"
                  : "Your monthly giving plan"
                : "Your giving plan"}
            </DialogTitle>
            <DialogDescription className="mt-1 text-sm leading-6 text-dash-muted">
              {plan
                ? `Giving year ${plan.yearLabel} · ${plan.strategyDescription}`
                : "Loading your plan..."}
            </DialogDescription>
          </div>
        </DialogHeader>
            ...
            <div>
              ...
              {finalCycle ? (
                <p className="mt-2 text-xs text-slate-500">
                  {isOpenEnded
                    ? `After ${formatPlanDate(finalCycle.dueDate)} the same ${cadenceWord(plan.cadence)}ly charge keeps the next giving year covered.`
              ...
              <label className="flex cursor-pointer items-start gap-3 rounded-xl border border-slate-200 px-4 py-3">
                ...
                <span className="text-sm">
                  ...
                  <span className="mt-0.5 block text-slate-600">
                    Keep paying monthly when the next giving year starts. You
