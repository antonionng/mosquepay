...
export async function GET(
  ...
  const province = await db.getProvinceById(id);
  if (!province) {
    return NextResponse.json({ error: "Province not found." }, { status: 404 });
  }
  const rows = await db.listLodgeAnnualReturns(id);
  const headers = [
    "lodge_name",
    "lodge_number",
    "active_members",
  ...
  return new NextResponse(csv, {
    headers: {
      ...
      "Content-Disposition": `attachment; filename="annual-returns-${province.slug}.csv"`,
    },
  });
}
