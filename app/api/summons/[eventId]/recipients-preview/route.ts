...
export async function GET(request: NextRequest, { params }: Params) {
  ...
  try {
    ...
    const [event, members, honoraryGuests, defaults, summons, overrides] =
      await Promise.all([
...
export async function PATCH(request: NextRequest, { params }: Params) {
  ...
  try {
    ...
    const existing = await db.getEventSummons(eventId, lodgeId);
    let summons = existing ?? null;
    if (Object.prototype.hasOwnProperty.call(body, "include_honorary_guests")) {
      ...
      summons = await db.upsertEventSummons(lodgeId, eventId, {
        ...(existing ?? {}),
    ...
    if (
      ...
      event = await db.updateEvent(eventId, lodgeId, {
        dining_waived_for_all: flag,
      });
    }

    return NextResponse.json({ summons, event });
  } catch (error) {
