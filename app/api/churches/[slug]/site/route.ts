...
export async function GET(
  ...
  if (isSupabaseConfigured()) {
    ...
    if (site && !site.published) {
      const forbidden = await requireAdminApiPermission("website:write", church.id);
      if (forbidden) {
...
export async function PATCH(
  ...
  try {
    ...
    if (isSupabaseConfigured()) {
      ...
      if (sectionsToSave) {
        const prev = await db.getChurchSite(churchId);
      ...
      const site = await db.updateChurchSite(churchId, {
        ...updates,
      ...
      await writeAuditLog({
        ...
        entityType: "website",
        entityId: site.id,
        summary: `Updated church website ${site.page_title}`,
      });
    ...
    let footerSettingsToSave = updates.footer_settings;
    if (sectionsToSave) {
      const prev = mockDb.getChurchSite(churchSlug);
      sectionsToSave = mergeSectionStylesPreserve(sectionsToSave, prev.sections);
    ...
    const site = mockDb.updateChurchSite(churchSlug, {
      ...updates,
      sections: sectionsToSave,
      custom_pages: customPagesToSave,
      header_settings: headerSettingsToSave,
