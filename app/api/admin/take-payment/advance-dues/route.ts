// POST /api/admin/take-payment/advance-dues
//
// Admin entry point for "treasurer takes an advance-dues payment for a
// member at the desk". Supports two methods:
//
//   method="card_qr"  Mints a Mooov hosted Checkout against the resolved
//                     advance member_dues row. The cardholder taps the
//                     QR; on capture the existing /api/mooov-webhooks
//                     dues projector flips the dues row paid.
//
//   method="cash"     Records a cash payment_attempts row + projects the
//                     public.payments row + flips the advance dues row
//                     to paid in a single sweep.
//
// All "resolve next year + create advance member_dues" logic delegates to
// lib/dues/advance.ts, the same helper the member-portal pay-in-advance
// route uses.
//
// GET /api/admin/take-payment/advance-dues?member_id=...
// Returns a preview of the resolved next-year dues amount + discount,
...
// ---------------------------------------------------------------------------
// GET — preview the advance dues amount without writing anything
// ---------------------------------------------------------------------------
...
export async function GET(request: NextRequest) {
  ...
  if (!currentYear) {
    return NextResponse.json({
      ...
      message: "Lodge has no current masonic year configured.",
    });
  }
  ...
  const existing = memberDues.find(
    (d) =>
      d.is_advance &&
      next != null &&
  ...
  return NextResponse.json({
    ...
    currency: (lodgeDues?.currency ?? "gbp").toUpperCase(),
    already_prepaid: existing != null,
    existing_dues_id: existing?.id ?? null,
  });
}

// ---------------------------------------------------------------------------
// POST — resolve dues row and either mint a Mooov QR or record cash
// ---------------------------------------------------------------------------
...
export async function POST(request: NextRequest) {
  ...
  const resolved = await resolveOrCreateAdvanceDues({
    lodgeId,
    member,
    amountOverride,
  });
  ...
  if (!resolved.already_existed) {
    await writeAuditLog({
      ...
      entityType: "dues",
      entityId: resolved.member_dues_id,
      summary: `Advance dues created at desk for ${member.email} (${resolved.next_year_label})`,
      metadata: {
  ...
  if (method === "card_qr") {
    return await mintCardQrForAdvanceDues({
      lodgeId,
      lodgeSlug,
      member,
      resolvedAmount: resolved.charged_amount,
      currency: resolved.currency,
      duesId: resolved.member_dues_id,
      nextYearLabel: resolved.next_year_label,
      clientToken,
    });
  }

  return await recordCashForAdvanceDues({
    lodgeId,
    lodgeSlug,
    member,
    resolved,
...
// ---------------------------------------------------------------------------
// card_qr: hosted Mooov Checkout against the resolved advance dues row.
// Webhook handler's existing dues projector flips the dues row paid on
// payment.captured because the body sets intent="dues" + dues_id.
// ---------------------------------------------------------------------------

async function mintCardQrForAdvanceDues(args: {
  lodgeId: string;
  lodgeSlug: string;
  ...
  if (
    ...
    return NextResponse.json(
      {
        error: "This lodge has not finished setting up online payments yet.",
  ...
  const paymentId = `pay_dues_advance_${args.duesId}_${Date.now().toString(36)}`;
  ...
  const description = `Advance dues — ${args.nextYearLabel}`;

  const guestDescriptor: Record<string, unknown> = {
    source: "admin_take_payment_advance_dues",
    dues_id: args.duesId,
    member_id: args.member.id,
    lodge_slug: args.lodgeSlug,
  ...
  const initialMetadata: Record<string, unknown> = {
    source: "lodgepay_admin_take_payment_advance_dues",
    lodge_slug: args.lodgeSlug,
  ...
  if (insertError) {
    console.error("advance dues qr mint: payment_attempts insert failed", {
      lodge_id: args.lodgeId,
      payment_id: paymentId,
  ...
  try {
    ...
    if (err instanceof MooovApiError) {
      console.error("advance dues qr mint: Mooov call failed", {
        payment_id: paymentId,
        status: err.status,
...
// ---------------------------------------------------------------------------
// cash: payment_attempts(captured) + project payments + flip dues paid
// ---------------------------------------------------------------------------

async function recordCashForAdvanceDues(args: {
  lodgeId: string;
  lodgeSlug: string;
  member: NonNullable<Awaited<ReturnType<typeof db.getMemberById>>>;
  resolved: Awaited<ReturnType<typeof resolveOrCreateAdvanceDues>>;
  clientToken: string;
  note: string;
}): Promise<NextResponse> {
  const { lodgeId, lodgeSlug, member, resolved } = args;
  const supa = createServiceClient();
  ...
  if (dupe) {
    return NextResponse.json({
      payment_id: dupe.payment_id,
      dues_id: resolved.member_dues_id,
      amount: dupe.amount,
  ...
  try {
    ...
    console.warn("advance dues cash: admin identity unresolved", {
  ...
  const paymentId = `cash_advance_${lodgeId}_${Date.now().toString(36)}_${Math.random()
    .toString(36)
    .slice(2, 10)}`;
  const description = `Advance dues — ${resolved.next_year_label}`;
  const capturedAt = new Date().toISOString();

  const metadata: Record<string, unknown> = {
    source: "lodgepay_admin_take_payment_advance_dues_cash",
  ...
  const { error: insertError } = await supa
    ...
    .insert({
      ...
      metadata,
      guest_descriptor: {
        source: "admin_take_payment_advance_dues_cash",
        lodge_slug: lodgeSlug,
        dues_id: resolved.member_dues_id,
        member_id: member.id,
  ...
  if (insertError) {
    console.error("advance dues cash: attempt insert failed", {
      lodge_id: lodgeId,
      message: insertError.message,
      code: insertError.code,
    });
  ...
  // Project the public.payments row + Gift Aid donation if the lodge
  ...
  const projection = await projectTakePaymentCaptured({
    ...
    reference: `Advance dues ${resolved.next_year_label}`,
    eventId: null,
    charityName: null,
  ...
  // Flip the advance dues row to paid.
  await db.updateMemberDuesStatus(resolved.member_dues_id, lodgeId, {
    status: "paid",
