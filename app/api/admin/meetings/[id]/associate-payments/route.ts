// POST /api/admin/meetings/[id]/associate-payments
//
// Bulk-link a set of payments to this meeting. Used by the meeting page's
// "payments taken the same day as this meeting" reconciliation panel so a
...
export async function POST(
  ...
  for (const paymentId of ids) {
    const payment = await db.getPaymentById(paymentId, lodgeId).catch(() => null);
    if (!payment) {
  ...
  await writeAuditLog({
    ...
    entityId: eventId,
    summary: detach
      ? `Detached ${associated.length} payment${associated.length === 1 ? "" : "s"} from meeting.`
      : `Associated ${associated.length} payment${associated.length === 1 ? "" : "s"} with meeting.`,
    metadata: {
