...
import { buildPublicUrl, churchScopedNewcomerPath } from "@/lib/public-links";
...
export async function POST(
  ...
  const motherChurchNumber = trimOrNull(body.mother_church_number);
  ...
  if (isSupabaseConfigured()) {
    ...
    if (!church || !church.accepts_self_registration) {
      return NextResponse.json(
        { error: "This church is not currently accepting public registrations." },
        { status: 404 }
      );
    }

    const existing = await db.findGuestByEmail(church.id, email);
    const guest = await db.upsertGuest(church.id, {
      full_name: fullName,
      email,
      phone,
      mother_church_name: ismember ? motherChurchName : null,
      mother_church_number: ismember ? motherChurchNumber : null,
    ...
    const url = buildNewcomerUrl(request, church.slug, newcomerToken);

    const upcoming = await db.getEvents(church.id, {
      published: true,
      upcoming: true,
    });
  ...
  const church = mockDb.getChurchBySlug(slug);
  if (!church || !church.accepts_self_registration) {
    return NextResponse.json(
      { error: "This church is not currently accepting public registrations." },
      { status: 404 }
