...
export async function POST(request: NextRequest) {
  ...
  const existingCollections = await db.getMeetingCollections(lodgeId, { taxYear });
  ...
  const collection = await db.createMeetingCollection(lodgeId, {
    ...
    title: typeof body.title === "string" && body.title.trim() ? body.title.trim() : "Meeting collection",
  ...
  await writeAuditLog({
    lodgeId,
    action: "meeting_collection_created",
    entityType: "meeting_collection",
    entityId: collection.id,
    summary: `Recorded meeting collection ${collection.title}`,
    metadata: {
