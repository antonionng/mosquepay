// PATCH /api/lodges/current-charity-campaign
//
// Sets (or clears) public.lodges.current_charity_campaign_id for the active
// lodge. The Almoner / Charity Steward uses this to designate "this year's
// lodge charity" — the dynamic resolver at /give/<slug>/charity defaults
...
export async function PATCH(request: NextRequest) {
  ...
  // lodge and is in an active/usable state. We don't surface 'paused' as
  ...
  if (campaignId) {
    const { data: campaign, error: campaignError } = await supa
      ...
      .select("id, lodge_id, status, name")
      .eq("id", campaignId)
      .maybeSingle<{
        id: string;
    ...
    if (campaign.lodge_id !== lodgeId) {
      return NextResponse.json(
        { error: "Campaign belongs to a different lodge." },
        { status: 403 },
      );
    }
  ...
  await writeAuditLog({
    ...
    entityType: "lodge_charity_designation",
    entityId: lodgeId,
    summary: campaignId
      ? `Designated charity campaign ${campaignId} as current`
      : "Cleared the lodge's designated current charity campaign",
