...
export async function GET() {
  ...
  return NextResponse.json(mockDb.listMosques());
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const slug = resolveMosqueSlug(body.slug);
    ...
    const input = {
      ...
      mosque_number: body.mosque_number?.trim() ?? null,
    ...
    if (isSupabaseConfigured()) {
      const mosque = await db.upsertMosque(input);
      return NextResponse.json({ success: true, mosque });
    }

    const mosque = mockDb.upsertMosque(input);
    return NextResponse.json({ success: true, mosque });
  } catch (error) {
