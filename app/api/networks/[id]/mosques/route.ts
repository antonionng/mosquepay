...
export async function POST(
  ...
  await db.setMosqueNetwork(body.mosque_id, id);
...
export async function DELETE(
  ...
  const mosques = await db.listMosquesByNetwork(networkId);
  if (!mosques.find((l) => l.id === mosqueId)) {
    return NextResponse.json({ error: "Mosque not in this network." }, { status: 400 });
  }
  await db.setMosqueNetwork(mosqueId, null);
  return NextResponse.json({ ok: true });
}
