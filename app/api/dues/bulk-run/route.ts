...
export async function POST(request: NextRequest) {
  try {
    ...
    const duesId: string | null = body.dues_id ?? null;
    ...
    if (duesId) {
      ...
      const template = lodgeDuesList.find((d) => d.id === duesId);
      if (!template) {
        return NextResponse.json(
          { error: "Lodge dues template not found." },
          { status: 404 }
        );
      }
    ...
    const allMembers = await db.getMembers(lodgeId, { status: "active" });
    ...
    const existing = await db.getMemberDues(lodgeId);
    ...
    const created: { dues_id: string; member_email: string; instalments: number }[] = [];
    ...
    for (const member of targets) {
      ...
      const dues = await db.createMemberDues(lodgeId, {
        member_email: member.email,
    ...
    await writeAuditLog({
      lodgeId,
      action: "bulk_dues_run",
      entityType: "dues",
      entityId: lodgeId,
      summary: `Bulk dues run created ${created.length} dues records`,
      metadata: {
        period_start: periodStart,
        period_end: periodEnd,
        amount,
    ...
    console.error("Bulk dues run error:", e);
    return NextResponse.json(
      { error: "Failed to run bulk dues." },
      { status: 500 }
    );
  }
}
