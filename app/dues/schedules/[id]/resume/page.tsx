...
//
// Flow:
//   1. Fetch /api/dues/schedules/[id]/sca → publishable_key,
...
export default function ScaResumePage({
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`/api/dues/schedules/${id}/sca`, {
          method: "GET",
          credentials: "include",
        });
        if (!res.ok) {
  ...
  return (
    <main className="mx-auto max-w-xl px-4 py-12">
      ...
      <p className="mb-6 text-sm text-muted-foreground">
        Your bank has asked us to verify this month&apos;s dues charge before
      ...
      {phase === "error" && (
        <div className="space-y-3 rounded-md border border-amber-200 bg-amber-50 p-4 text-sm">
          ...
          <p className="text-amber-800">
            {errorMessage ??
              "Please try again, or contact your lodge secretary if the problem persists."}
          </p>
          <Button
            variant="outline"
            size="sm"
            onClick={() => {
              setPhase("loading");
              setErrorMessage(null);
              fetch(`/api/dues/schedules/${id}/sca`, {
                method: "GET",
                credentials: "include",
              })
