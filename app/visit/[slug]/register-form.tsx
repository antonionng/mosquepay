...
const schema = z.object({
  ...
  is_mason: z.boolean().default(false),
  mother_lodge_name: z.string().optional(),
...
export function VisitorRegisterForm({ lodgeSlug }: Props) {
  ...
  const {
    ...
    defaultValues: { is_mason: false },
  });

  const isMason = watch("is_mason");

  async function onSubmit(data: FormData) {
    setError(null);
    try {
      const res = await fetch(
        `/api/visit/${encodeURIComponent(lodgeSlug)}/register`,
        {
          method: "POST",
      ...
      <label className="flex items-start gap-3 rounded-2xl border border-slate-200 bg-slate-50 p-4">
        ...
        <span>
          ...
          <span className="mt-1 block text-sm text-slate-500">
            Tick this if you would like to be considered for visiting brethren
            events (blue table).
          </span>
        </span>
      </label>
