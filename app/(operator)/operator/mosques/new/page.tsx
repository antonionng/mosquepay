...
const STEPS = [
  { label: "Mosque Details", icon: Building2 },
...
const initial: FormData = {
  ...
  ai_audience: "Prospective mosque member newcomers",
};
...
export default function MosqueOnboardingWizard() {
  ...
  async function handleSubmit() {
    ...
    setError(null);
    try {
      const mosqueRes = await fetch("/api/mosques", {
        method: "POST",
      ...
      if (!mosqueRes.ok) {
        const body = await mosqueRes.json().catch(() => ({}));
        throw new Error(body.error || "Failed to create mosque");
      }
      const { mosque } = await mosqueRes.json();

      if (form.ai_brief.trim()) {
        await fetch(`/api/mosques/${mosque.slug}/ai-draft`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            brief: form.ai_brief,
      ...
      router.push(`/operator/mosques/${mosque.slug}`);
    } catch (e) {
  ...
  return (
    <div className="space-y-8">
      <div className="admin-page-head">
        <div>
          <h1 className="admin-page-title">Onboard New Mosque</h1>
          <p className="admin-page-copy">
            Set up a new mosque on the platform
          </p>
        </div>
      </div>
                ...
                <div className="grid gap-5 sm:grid-cols-2">
                  ...
                  <div className="sm:col-span-2">
                    ...
                    <input
                      type="email"
                      className={inputCls}
                      placeholder="secretary@mosque.com"
                ...
                <div className="grid gap-5">
                  <div>
                    ...
                    <textarea
                      ...
                      placeholder="Describe the mosque, its history, and what makes it special…"
                      value={form.ai_brief}
                  ...
                  <div className="grid gap-5 sm:grid-cols-2">
                    ...
                    <div>
                      ...
                      >
                        <option value="Prospective mosque member newcomers">
                        ...
                        <option value="Mosque newcomers and guests">
                  ...
                  {!form.ai_brief.trim() && (
                    <p className="rounded-xl border border-amber-500/20 bg-amber-500/5 px-4 py-3 text-sm text-amber-300">
                      You can skip the AI website step. The mosque will be
          ...
          {step < 3 ? (
            ...
            >
              {submitting ? (
                ...
                <>
                  <CheckCircle2 className="h-4 w-4" /> Create Mosque
                </>
              )}
            </button>
          )}
