...
import { MosqueDetailActions } from "./actions";
import { requireOperatorPageAccess } from "@/lib/auth/operator-page";

type Props = { params: Promise<{ slug: string }> };

async function getMosqueData(slug: string) {
  ...
  if (isSupabaseConfigured()) {
    mosque = await db.getMosqueBySlug(slug).catch(() => null);
    if (!mosque) {
      const allMosques = await db.listMosques().catch(() => []);
      mosque = allMosques.find((l) => l.slug === slug) ?? null;
    }
    if (mosque) {
      subscription = await db.getMosqueSubscription(mosque.id).catch(() => null);
      const events = await db.getEvents(mosque.id).catch(() => []);
      eventsCount = events.length;
      const newcomers = await db.getNewcomers(mosque.id).catch(() => []);
      newcomersCount = newcomers.length;
      const payments = await db.getPayments(mosque.id).catch(() => []);
      paymentsTotal = payments.reduce((sum, p) => sum + p.total_amount, 0);
    }
  } else if (shouldUseInMemoryMock()) {
    const allMosques = mockDb.listMosques();
    mosque = allMosques.find((l: Mosque) => l.slug === slug) ?? null;
  }

  return { mosque, subscription, eventsCount, newcomersCount, paymentsTotal };
}
...
export default async function MosqueDetailPage({ params }: Props) {
  ...
  const { mosque, subscription, eventsCount, newcomersCount, paymentsTotal } =
    await getMosqueData(slug);
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        >
          <ArrowLeft className="h-3.5 w-3.5" /> Back to Mosques
        </Link>
        <div className="admin-page-head">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="admin-page-title">{mosque.name}</h1>
              <div className="flex flex-wrap items-center gap-3 mt-1">
                {mosque.city && (
                  <span className="flex items-center gap-1 text-sm text-slate-400">
                    <MapPin className="h-3.5 w-3.5" /> {mosque.city}
          ...
          <div className="flex items-center gap-2">
            <Link
              href={`/admin/settings?mosque=${mosque.slug}`}
      ...
      <div className="grid gap-6 lg:grid-cols-3">
          ...
          <div className="admin-surface p-6">
            ...
            <div className="grid gap-4 sm:grid-cols-2 mt-4">
              {[
                ["Name", mosque.name],
                ["Slug", mosque.slug],
                ["City", mosque.city ?? "Not recorded"],
                ["Country", mosque.country ?? "Not recorded"],
                ["Tagline", mosque.tagline ?? "Not recorded"],
              ].map(([label, value]) => (
                <div key={label}>
              ...
              <div>
                ...
                <p className="mt-0.5 text-sm text-white">
                  {mosque.support_email ? (
                    ...
                    >
                      <Mail className="h-3 w-3" /> {mosque.support_email}
                    </a>
                  ) : (
                    "Not recorded"
                  )}
              ...
              <div>
                ...
                <div className="mt-1.5 flex gap-2">
                  <div
                    ...
                    style={{
                      backgroundColor: mosque.primary_color ?? "#1e3a5f",
                    }}
                  />
                  <div
                    className="h-6 w-6 rounded-md border border-white/10"
                    style={{
                      backgroundColor: mosque.secondary_color ?? "#d4af37",
                    }}
                  />
                </div>
              </div>
        ...
        <div className="space-y-6">
          ...
          <MosqueDetailActions mosque={mosque} />
        </div>
      </div>
    </div>
  );
