...
const STEPS = [
  { label: "Lodge Details", icon: Building2 },
...
const initial: FormData = {
  ...
  ai_audience: "Prospective Freemason candidates",
};

export default function LodgeOnboardingWizard() {
  ...
  async function handleSubmit() {
    ...
    setError(null);
    try {
      const lodgeRes = await fetch("/api/lodges", {
        method: "POST",
      ...
      if (!lodgeRes.ok) {
        const body = await lodgeRes.json().catch(() => ({}));
        throw new Error(body.error || "Failed to create lodge");
      }
      const { lodge } = await lodgeRes.json();

      if (form.ai_brief.trim()) {
        await fetch(`/api/lodges/${lodge.slug}/ai-draft`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            brief: form.ai_brief,
      ...
      router.push(`/operator/lodges/${lodge.slug}`);
    } catch (e) {
  ...
  return (
    <div className="space-y-8">
      <div className="admin-page-head">
        <div>
          <h1 className="admin-page-title">Onboard New Lodge</h1>
          <p className="admin-page-copy">
            Set up a new lodge on the platform
          </p>
        </div>
      </div>
                ...
                <div className="grid gap-5 sm:grid-cols-2">
                  ...
                  <div className="sm:col-span-2">
                    ...
                    <input
                      type="email"
                      className={inputCls}
                      placeholder="secretary@lodge.com"
                ...
                <div className="grid gap-5">
                  <div>
                    ...
                    <textarea
                      ...
                      placeholder="Describe the lodge, its history, and what makes it special…"
                      value={form.ai_brief}
                  ...
                  <div className="grid gap-5 sm:grid-cols-2">
                    ...
                    <div>
                      ...
                      >
                        <option value="Prospective Freemason candidates">
                        ...
                        <option value="Lodge visitors and guests">
                          Visitors and guests
                        </option>
                      </select>
                    </div>
          ...
          {step < 3 ? (
            ...
            >
              {submitting ? (
                ...
                <>
                  <CheckCircle2 className="h-4 w-4" /> Create Lodge
                </>
              )}
            </button>
          )}
