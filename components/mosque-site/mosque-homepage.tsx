...
import type { MosqueSiteFooterSettings, MosqueSiteHeaderSettings } from "@/lib/db/types";
...
function withQuery(href: string, mosqueSlug: string) {
  if (href.startsWith("#")) return href;
  return `${href}?mosque=${encodeURIComponent(mosqueSlug)}`;
}
...
function ContactEnquiryForm({
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch(`/api/contact?mosque=${encodeURIComponent(mosqueSlug)}`, {
        method: "POST",
        ...
        body: JSON.stringify({
          ...
          subject: form.get("subject") || `Website enquiry for ${mosque?.name ?? "the mosque"}`,
      ...
      setStatus(formThankYou(section, "Thanks. Your message has been sent to the mosque secretary."));
    } catch (error) {
      ...
      {formFieldVisible(section, "message") ? (
        <div className="space-y-2">
          ...
          <Textarea
            ...
            placeholder="Tell us how the mosque can help..."
            rows={5}
          />
        </div>
      ) : null}
...
function NewcomerIntakeForm({
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const form = new FormData(event.currentTarget);
    try {
      const response = await fetch(`/api/newcomers?mosque=${encodeURIComponent(mosqueSlug)}`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          section_id: section.id,
      ...
      event.currentTarget.reset();
      setStatus(formThankYou(section, "Thanks. Your enquiry has been added and sent to the mosque secretary."));
    } catch (error) {
    ...
    >
      <div className="mb-6">
        ...
        <p className={dark ? "mt-2 text-sm text-slate-300" : "mt-2 text-sm text-slate-600"}>
          {mode === "newcomer"
            ? "This creates a newcomer in the mosque CRM and notifies the mosque secretary."
            : "This sends your message directly to the mosque secretary or support contact."}
        </p>
      </div>
      {mode === "newcomer" ? (
        <NewcomerIntakeForm mosqueSlug={mosqueSlug} section={section} dark={dark} />
      ) : (
        <ContactEnquiryForm mosqueSlug={mosqueSlug} mosque={mosque} section={section} dark={dark} />
      )}
    </div>
  );
}
  ...
  return (
    <section className="border-y border-slate-200 bg-slate-50 py-16">
      <div className="container-full">
        <SectionFormCard mode={mode} mosqueSlug={mosqueSlug} mosque={mosque} section={section} />
      </div>
    </section>
  );
}
...
function HeroSection({
  ...
  mosque: MosqueData | null;
  mosqueSlug: string;
  footerSettings: MosqueSiteFooterSettings | null;
}) {
  const layers = heroBackgroundLayers(section);
  const primaryHex = mergeHeroPrimaryColor(section, mosque?.primary_color);
  const hasCustomBg = Boolean(layers.imageUrl);
  const showPoweredBy = footerSettings?.show_powered_by !== false;
  const hasCharity = Boolean(mosque?.current_charity_campaign_id);
    ...
    >
      ...
      >
        <div className="public-hero-copy mx-auto text-center lg:text-center">
          <FadeIn delay={0.1}>
            <p className="public-kicker mx-auto inline-flex">{mosque?.name ?? "Welcome"}</p>
          </FadeIn>
          <FadeIn delay={0.2}>
          ...
          <FadeIn delay={0.4}>
            <div className="mt-10 flex flex-wrap items-center justify-center gap-4">
              {section.cta_href && section.cta_label && (
                ...
                >
                  <Link href={withQuery(section.cta_href, mosqueSlug)}>
                    {section.cta_label}
                    <ArrowRight className="ml-2 h-4 w-4" />
                  </Link>
                </Button>
              ...
              {hasCharity ? (
                ...
                >
                  <Link href={withQuery("/charity", mosqueSlug)}>Our Charity</Link>
                </Button>
              ) : null}
            </div>
          </FadeIn>
          <FadeIn delay={0.5}>
            <div className="public-hero-meta mt-12 justify-center">
              {mosque?.city && <span className="public-hero-meta-chip">{mosque.city}</span>}
    ...
    >
      ...
      <div className={`container-full ${sectionContentClass(section)}`} style={sectionContainerStyle(section)}>
        <div className="grid gap-12 lg:grid-cols-[1fr_0.6fr] lg:items-center">
          <FadeIn>
            <div>
              <p className="section-label">About the mosque</p>
...
function ServiceDetailsSection({
  ...
  const venue = mosque?.service_location ?? mosque?.secretary_address ?? null;
  const cards: {
    icon: ReactNode;
    label: string;
    value: string;
  ...
  if (mosque?.service_schedule) {
    cards.push({
      icon: <Calendar className="h-6 w-6" />,
      label: "Schedule",
      value: mosque.service_schedule,
    });
  }
  if (venue) {
    cards.push({
      ...
      label: "Where we meet",
      value: venue,
      href: mosque?.service_location_url ?? undefined,
      helper: mosque?.accessibility_notes ?? null,
    });
  }
  if (mosque?.default_dress_code) {
    cards.push({
      icon: <Users className="h-6 w-6" />,
      label: "Dress code",
      value: mosque.default_dress_code,
    });
  }
  if (mosque?.loi_contact) {
    cards.push({
      icon: <Clock className="h-6 w-6" />,
      label: "Study Circle",
      value: mosque.loi_contact,
    });
  }
  return (
    <section
...
function OfficersSection({
  ...
  // image above so the mosque can still describe its officers in their
...
function CharitySection({
  ...
  // charity campaign via mosques.current_charity_campaign_id. Otherwise
          ...
          <FadeIn delay={0.15}>
            {campaign ? (
              <div className="public-grid-card">
                ...
                >
                  <Link href={withQuery("/donate", mosqueSlug)}>
...
function EventsSection({
  ...
  // Regular mosque services are private. This grid is populated only with
  ...
  // mosque never advertises a fake calendar.
          ...
          <StaggerChildren className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3" staggerDelay={0.08}>
            {events.map((event) => (
              <StaggerItem key={event.id}>
                <Link
                  href={withQuery(`/events/${event.slug}`, mosqueSlug)}
...
function FaqSection({ section }: { section: SiteSection }) {
  // FAQ entries are configured per-mosque (Phase 1 adds the admin editor).
  // When no entries exist we keep the heading/body/image but drop the
  // accordion so we never invent answers on behalf of the mosque.
    ...
    >
      ...
      <div className="container-full relative z-10 text-center">
        <FadeIn>
          ...
          {mode !== "none" ? (
            <SectionFormCard mode={mode} mosqueSlug={mosqueSlug} mosque={mosque} section={section} dark />
          ) : null}
          <div className="mt-10 flex flex-wrap items-center justify-center gap-4">
            {section.cta_href && section.cta_label && (
            ...
            <Button
              ...
              className="rounded-xl border-white/10 bg-white/10 text-white hover:bg-white/20 hover:text-white"
            >
              <Link href={withQuery("/contact", mosqueSlug)}>Contact Us</Link>
            </Button>
          </div>
        </FadeIn>
      </div>
    ...
    >
      ...
      <div className={`container-full ${sectionContentClass(section)}`} style={sectionContainerStyle(section)}>
        <div className="grid gap-14 lg:grid-cols-[1fr_0.8fr]">
          <FadeIn>
            <div>
              ...
              {mode !== "none" ? (
                <div className="mt-10">
                  {mode === "newcomer" ? (
                    <NewcomerIntakeForm mosqueSlug={mosqueSlug} section={section} />
                  ) : (
                    <ContactEnquiryForm mosqueSlug={mosqueSlug} mosque={mosque} section={section} />
                  )}
                </div>
              ) : null}
            </div>
          ...
          {(() => {
            const hasAddress = Boolean(mosque?.secretary_address);
            const hasEmail = Boolean(mosque?.support_email);
            const hasPhone = Boolean(mosque?.support_phone);
            if (!hasAddress && !hasEmail && !hasPhone) return null;
            return (
              <FadeIn delay={0.15}>
                <div className="public-grid-card-muted h-fit lg:mt-20">
                  <h3 className="text-lg font-semibold text-slate-950">Mosque details</h3>
                  <div className="mt-6 space-y-5">
                    {hasAddress && mosque?.secretary_address ? (
...
function SectionRenderer({
  ...
  switch (section.type) {
    ...
    case "faq":
      ...
      break;
    case "join":
      rendered = <JoinSection section={section} mosqueSlug={mosqueSlug} mosque={mosque} />;
      break;
    case "contact":
      rendered = <ContactSection section={section} mosque={mosque} mosqueSlug={mosqueSlug} />;
      break;
    default:
  ...
  return (
    <>
      {rendered}
      <StandaloneSectionForm section={section} mosqueSlug={mosqueSlug} mosque={mosque} />
    </>
  );
}

/* ────────────────────────────────────────────────────────────
 *  MAIN COMPONENT: MosqueHomepage
 * ──────────────────────────────────────────────────────────── */
export function MosqueHomepage({
  pageSlug = "home",
  initialMosqueSlug,
  initialPayload = null,
}: {
  pageSlug?: string;
  initialMosqueSlug?: string | null;
  initialPayload?: SitePayload | null;
}) {
  const searchParams = useSearchParams();
  const rawMosqueQuery = searchParams.get("mosque");
  const mosqueSlug = useMemo(
    () => initialMosqueSlug ?? resolveMosqueSlug(rawMosqueQuery),
    [initialMosqueSlug, rawMosqueQuery]
  );
  const [payload, setPayload] = useState<SitePayload | null>(initialPayload);
  const [loading, setLoading] = useState(!initialPayload);

  useEffect(() => {
    if (initialPayload && initialMosqueSlug === mosqueSlug) return;
  ...
  }, [initialPayload, initialMosqueSlug, mosqueSlug, rawMosqueQuery]);
    ...
    return (
      <div className="flex min-h-[60vh] items-center justify-center bg-white">
        <div className="text-center">
          ...
          <p className="mt-4 text-sm text-slate-500">Loading mosque website...</p>
        </div>
      </div>
    );
  }
  ...
  if (!payload) {
    return (
      <div className="flex min-h-[60vh] items-center justify-center bg-white">
        <div className="text-center">
          <h2 className="text-2xl font-semibold text-slate-950">Mosque not found</h2>
          <p className="mt-2 text-slate-600">We couldn&apos;t load this mosque website.</p>
    ...
    return (
      <div className="flex min-h-[60vh] items-center justify-center bg-white">
        <div className="text-center">
          ...
          <p className="mt-2 text-slate-600">This mosque page is not published.</p>
          <Button asChild className="mt-6" variant="outline">
            <Link href={`/?mosque=${encodeURIComponent(mosqueSlug)}`}>Return to mosque home</Link>
          </Button>
        </div>
      </div>
    );
        ...
        <section className="relative overflow-hidden bg-slate-950 py-20" aria-label="Get involved">
          ...
          <div className="container-full relative z-10 text-center">
            <FadeIn>
              ...
              >
                <Link href={withQuery("/join", mosqueSlug)}>
                  Express Interest
                  <ArrowRight className="ml-2 h-4 w-4" />
                </Link>
              </Button>
