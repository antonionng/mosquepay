...
export function SummonsRecipientsPanel({ eventId }: Props) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/summons/${eventId}/recipients-preview?${params.toString()}`
      );
  ...
  async function persistHonoraryToggle(checked: boolean) {
    ...
    try {
      await fetch(`/api/summons/${eventId}/recipients-preview`, {
        method: "PATCH",
  ...
  async function persistDiningWaiveAll(checked: boolean) {
    ...
    try {
      const res = await fetch(`/api/summons/${eventId}/recipients-preview`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-dash-muted">
            Everyone who will be invited when the summons is sent, with the
        ...
        <label className="flex items-start gap-2 rounded-lg border border-amber-200 bg-amber-50/60 p-3 text-sm text-amber-900">
          ...
          <div>
            ...
            <p className="text-xs text-amber-800">
              Useful when the lodge is covering dining (e.g. installation). This
...
function OverrideDialog({
  ...
  async function save() {
    ...
    try {
      ...
      if (allDefault) {
        const res = await fetch(
          `/api/summons/${eventId}/overrides?subject_type=${subjectType}&subject_id=${row.id}`,
          { method: "DELETE" }
        );
        if (!res.ok) {
        ...
        const res = await fetch(`/api/summons/${eventId}/overrides`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
    ...
    >
      ...
      >
        <DialogHeader className="space-y-2 border-b border-dash-border px-6 py-5">
          ...
          <DialogDescription>
            This adjustment applies only to this meeting. The recipient&apos;s
        ...
        <div className="space-y-5 px-6 py-5">
          ...
          <div className="space-y-1">
            ...
            <Textarea
              id="override-note"
              rows={2}
              placeholder="e.g. Secretary's guest, lodge covering dining."
              value={note}
              maxLength={500}
