...
// components/admin/dues-method-panel.tsx
//
// Admin-side panel on /admin/members/[id] that surfaces *how* the
// member is paying this year's dues and lets the treasurer / secretary
...
// member_dues.dues_payment_method (added in migration 063).
...
import type { DuesPaymentMethod } from "@/lib/db/types";

export interface DuesMethodPanelProps {
  memberId: string;
  memberEmail: string;
  memberName: string;
  initial: {
...
function methodLabel(method: DuesPaymentMethod | null): string {
  switch (method) {
...
function methodDescription(method: DuesPaymentMethod | null): string {
  switch (method) {
    case "online_subscription":
      return "Paying via card subscription through pay.mooov.money. Charges run automatically each cycle.";
    case "bacs":
      return "Pays by external bank transfer. Reconcile manually from the bank statement against the dues period.";
    case "paid_in_full":
      return "Paid in full offline (cash, cheque, transfer). Status recorded as paid for the year.";
    case "fee_waived":
      return "Lodge has waived dues for this year for this member.";
    default:
...
export function DuesMethodPanel({
  memberId,
  memberEmail,
  memberName,
  initial,
}: DuesMethodPanelProps) {
  ...
  const hasDuesRow = initial.duesId != null;
  ...
  async function callApi(payload: Record<string, unknown>) {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/admin/members/${memberId}/dues-method`, {
        method: "POST",
      ...
      setState({
        method: data.dues.dues_payment_method,
        bacsMonthlyAmount: data.dues.bacs_monthly_amount,
        bacsReference: data.dues.bacs_reference,
        waiverReason: data.dues.waiver_reason,
        paidAt: data.dues.paid_at,
        setBy: data.dues.payment_method_set_by,
        setAt: data.dues.payment_method_set_at,
  ...
  const mailtoHref = state.subscriptionLink
    ? `mailto:${memberEmail}?subject=${encodeURIComponent(
        `Lodge dues – set up your payment plan`,
      )}&body=${encodeURIComponent(
        `Hi ${memberName.split(" ")[0] ?? memberName},\n\n` +
          `You can set up your lodge dues here:\n\n${state.subscriptionLink}\n\n` +
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        {!hasDuesRow ? (
          <div className="rounded-xl border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900">
            <div className="flex items-start gap-2">
              ...
              <p>
                No dues record exists for this member yet. Create one from
                the <span className="font-semibold">Membership Dues</span>{" "}
            ...
            <div className="grid grid-cols-1 gap-3 text-sm md:grid-cols-2">
              ...
              {state.method === "bacs" ? (
                ...
                <div className="rounded-xl border border-blue-200 bg-blue-50/70 p-3">
                  ...
                  <p className="mt-1 text-sm text-slate-700">
                    Member has an active dues subscription — see the panel above for cycle history.
                  </p>
                </div>
              ) : (
                <div className="rounded-xl border border-slate-200 bg-slate-50/60 p-3">
      ...
      >
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Email {memberName} a link they can use to set up online dues.
      ...
      >
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>Mark dues as paid in full</DialogTitle>
            <DialogDescription>
      ...
      >
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>Waive dues for this member</DialogTitle>
            <DialogDescription>
              The lodge has agreed to waive {memberName}&apos;s dues for
              {initial.yearLabel ? ` ${initial.yearLabel}` : " this year"}.
          ...
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              {state.method === "paid_in_full" || state.method === "fee_waived"
                ? " The dues row's paid/waived status will be left in place — change it from the Membership Dues panel if needed."
                : ""}
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
