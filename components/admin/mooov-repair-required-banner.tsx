// Surfaces Mooov's `account_invalid` payment.failed signal to the lodge admin:
// the lodge's underlying PSP connection got severed (typically the lodge
...
// lodge admin has to walk through Mooov's portal repair flow.
//
// Persisted into mooov.lodges.status='needs_repair' by
...
export interface MooovRepairRequiredHint {
  ...
  // back. Must be on Mooov's return_url allowlist (today: lodgepayments.co.uk,
  // www.lodgepayments.co.uk, *.vercel.app).
  returnUrl: string;
...
export function MooovRepairRequiredBanner({
  ...
  repairUrl.searchParams.set("from", "lodgepay");
      ...
      <div className="flex flex-col gap-4 p-6 md:flex-row md:items-start md:gap-6">
        <div className="flex-1 space-y-2">
          ...
          <p className="text-sm leading-relaxed text-amber-800">
            We tried to take a payment through this lodge&rsquo;s Mooov merchant
            and the underlying payment account rejected the request. New dues,
