...
function statusLabel(row: NextDuesRow) {
  if (row.status === "waived_at_profile") return "Annual dues waived";
  if (row.status === "no_masonic_year") return "Year not configured";
...
export function NextDuesPanel() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const res = await fetch("/api/dues/next-due");
        const body = await res.json();
        if (!res.ok) throw new Error(body.error ?? "Could not load next dues.");
        if (!cancelled) setData(body);
      } catch (err) {
        if (!cancelled) {
          setError(err instanceof Error ? err.message : "Could not load next dues.");
        }
      } finally {
    ...
    <section className="space-y-4 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            When each active member&apos;s next annual dues bill falls due, aligned to the lodge&apos;s
            masonic year.
          </p>
        </div>
        {data?.year ? (
          <div className="text-right text-xs text-slate-500">
            ...
            {data.year.annual_dues_amount != null && (
              <p>Annual £{data.year.annual_dues_amount.toFixed(2)}</p>
            )}
          </div>
        ) : (
          <Link
            href="/admin/treasurer?tab=masonic-year"
            className="text-xs text-amber-700 underline"
          >
            No masonic year set — configure to drive next-due dates
          </Link>
        )}
      </div>
      ...
      <div className="flex flex-wrap items-end gap-3">
        <div className="flex-1 min-w-[200px] space-y-1">
          <Label htmlFor="next-dues-search">Search</Label>
          <Input
            id="next-dues-search"
        ...
        <div className="space-y-1">
          <Label htmlFor="next-dues-filter">Filter</Label>
          <select
            id="next-dues-filter"
            value={statusFilter}
            onChange={(e) =>
