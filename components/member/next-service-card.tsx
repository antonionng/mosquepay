...
export function NextServiceCard({ nextEvent }: { nextEvent: NextEvent | null }) {
    ...
    return (
      <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-semibold text-slate-900">No services scheduled</p>
            <p className="text-xs text-slate-500">
              You will see your next service here when it is published.
            </p>
          </div>
        </div>
      </div>
  ...
  async function submit(action: "attending" | "apologies") {
    ...
    try {
      ...
      setFeedback(
        action === "attending"
          ? "You are confirmed for this service."
      ...
      <div className="space-y-3 px-6 py-5">
        {!nextEvent.enable_rsvp ? (
          <p className="text-sm text-slate-500">
            RSVP is not enabled for this service yet.
          </p>
        ) : (
          <>
